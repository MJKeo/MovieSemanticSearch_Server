"""
Movie data model classes and enums.

This module contains all Pydantic models and Enum classes used for movie data
...
class MaturityRating(Enum):
    ...
    def from_string(cls, string: str):
        ...
        Returns:
            Matching MaturityRating enum or NR as default
        """
        for rating in cls:
...
class IMDBMovie(BaseModel):
    ...
    def plot_string(self) -> str:
        """
        Returns overview as-is for vector search (4.2).
        
        Returns:
            Overview text unchanged
        """
        # Access attribute directly to avoid method/attribute name conflict
        parts = []
        if self.synopsis:
            parts.append(f"Synopsis: {self.synopsis}")
        if self.plot_keywords:
            parts.append(f"{", ".join(self.plot_keywords)}")
        return "\n".join(parts)

    def genres_string(self) -> list[str]:
        """
