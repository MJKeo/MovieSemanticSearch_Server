...
class TraitDecomposition(BaseModel):
    ...
    routing_exploration: str = Field(
        ...,
        description=(
            "EXPLORATORY THINKING — produced AFTER dimensions and "
            "BEFORE combine_mode. 3-6 sentences walking the candidate "
            "set as a whole, deciding which categories deserve a commit "
            "and how those commits relate, BEFORE combine_mode and "
            "category_calls below read your conclusions.\n"
            "\n"
            "EXPECT FILLER IN THE CANDIDATE SET. The per-dimension "
            "category_candidates list was held to a floor of 5 per "
            "dimension to force broad exploration — this deliberately "
            "produces some filler candidates whose what_this_covers is "
            "thin or whose what_this_misses admits the candidate "
            "doesn't really fit. Your job here is to identify which "
            "candidates are real adjacencies (substantive coverage, "
            "named gap) vs filler (vague coverage, indefensible "
            "presence) and discard the filler. The dimensions and "
            "candidate lists are ROUGH input that this step refines "
            "into a committable routing decision — be ruthless about "
            "pruning anything that doesn't materially help retrieval.\n"
            "\n"
            "FORWARD REASONING, NOT BACK-RATIONALIZATION. Phrase as "
            "\"I see X across these dimensions, so I'm leaning toward "
            "Y\" rather than \"My answer is Y because X\". The fields "
            "below copy your conclusions; the conclusions themselves "
            "are reached here.\n"
            "\n"
            "Walk these four checks in order:\n"
            "\n"
            "(1) DEDUP. Look at category_candidates across all "
            "dimensions. When the same category appears as a clean-fit "
            "candidate on more than one dimension, note it — those "
            "dimensions will merge into one multi-expression call.\n"
            "\n"
            "(2) GRANULARITY GATE. For each candidate still in play, "
            "ask whether the aspect it serves is at the granularity "
            "the category retrieves. Identity categories — those that "
            "retrieve a specific named entity (a particular franchise, "
            "character, studio, person, source creator) — are "
            "admissible ONLY when the aspect itself names a specific "
            "entity that lives in that category. When the aspect "
            "names a CATEGORY (a genre, an archetype, a kind-of-movie "
            "defined by what its members share), identity-category "
            "candidates are out-of-scope no matter how cleanly prior "
            "knowledge associates specific named instances with that "
            "category. Drop them here. The discriminator: does the "
            "aspect name a MEMBER of the category or name the "
            "category itself?\n"
            "\n"
            "(3) COVERAGE. After dedup and granularity pruning, ask "
            "whether a single surviving category cleanly covers "
            "every dimension this trait routes — read the per-"
            "dimension what_this_misses analysis as the source of "
            "truth on whether real gaps remain. When one candidate's "
            "what_this_misses shows no substantive gap across every "
            "dimension it could serve, that category is sufficient "
            "evidence on its own; the trait is SOLO. Other "
            "candidates whose what_this_misses named real gaps were "
            "adjacency context, not coverage the primary lacks — "
            "they do not earn a commit under SOLO. Only when no "
            "single category covers everything cleanly does the "
            "relationship question in step (4) apply.\n"
            "\n"
            "(4) RELATIONSHIP & MINIMUM SET (only when no SOLO "
            "candidate exists). Look at the surviving candidates "
            "across dimensions: are they alternative HOMES for one "
            "underlying thing the trait names (matching any one is "
            "sufficient evidence → leans FRAMINGS), or do they "
            "describe DIFFERENT axes of a compound concept the user "
            "wants compounded (all must fire to a degree → leans "
            "FACETS)? Argue for one direction based on what the "
            "dimensions actually say. Then state which categories "
            "you will commit and which dimensions each owns. Each "
            "surviving category must add NEW signal — if a category "
            "is redundant with a stronger sibling and FRAMINGS gives "
            "no incremental reinforcement (or FACETS would double-"
            "fire on the same axis), drop it.\n"
            "\n"
            "CATEGORICAL-TO-SPECIFIC DRIFT — the named failure mode "
            "this field exists to catch. When the trait names a "
            "category, do not commit a category whose job is to "
            "retrieve specific named instances of that category. The "
            "category is broader than any single instance; the "
            "specific instance covers only a part. Adding multiple "
            "specific instances as 'framings' does not heal the gap "
            "— it narrows the scope to a hand-picked subset. This "
            "applies to both polarities.\n"
            "\n"
            "NEVER:\n"
            "- WRITE BACKWARD FROM A COMMITTED ANSWER. The reasoning "
            "leads the conclusion, not justifies it.\n"
            "- ELABORATE BEYOND THE CANDIDATES. Only categories "
            "surfaced in the candidate analysis above are in scope.\n"
            "- INTRODUCE NEW NAMED ENTITIES. The trait either named "
            "the entity or it didn't. Reaching for a particular "
            "franchise / character / studio / person name to justify "
            "a candidate is the Categorical-to-Specific Drift trap "
            "firing.\n"
            "- COMMIT EXTRA CATEGORIES AS HEDGES. When a clean "
            "primary covers the trait, partial-coverage adjacents do "
            "not add retrieval signal; they pad the call list. The "
            "commit is the minimum sufficient set, not the broadest "
            "defensible set.\n"
            "- LEAVE GENERIC. \"There are several candidates; I'll "
            "pick the best fits\" is a non-analysis."
        ),
    )
    combine_mode: TraitCombineMode = Field(
        ...
        description=(
            ...
            "the routing_exploration conclusion you committed just "
            "above — this field is the mechanical translation of that "
            "prose into the closed enum.\n"
            "\n"
            ...
            "three-way pick. Ask the coverage question first: does "
            "one surviving category cover every dimension this trait "
            "routes without a real gap? If yes → SOLO; the other "
            "candidates do not earn a commit. Only when the answer "
            "is no does the FRAMINGS-vs-FACETS question come into "
            "play (alternative homes for one signal → FRAMINGS; "
            "distinct compounding axes → FACETS).\n"
            "\n"
            "NEVER:\n"
            "- DIVERGE FROM routing_exploration. If the exploration "
    ...
    category_calls: list[CategoryCall] = Field(
        ...
        description=(
            ...
            "all reasoned through in routing_exploration above — this "
            ...
            "alongside the real adjacencies. The routing_exploration "
            ...
            "make the routing_exploration's surviving set, it MUST "
