...
class TraitDecomposition(BaseModel):
    ...
    routing_exploration: str = Field(
        ...
        description=(
            ...
            "category itself?\n"
            "\n"
            "(3) RELATIONSHIP. After dedup and granularity pruning, "
            "look at the surviving candidates across dimensions. Are "
            "they alternative HOMES for one underlying thing the "
            "trait names (matching any one is sufficient evidence → "
            "leans FRAMINGS), or do they describe DIFFERENT axes of "
            "a compound concept the user wants compounded (all must "
            "fire to a degree → leans FACETS)? Argue for one "
            "direction based on what the dimensions actually say.\n"
            "\n"
            "(4) MINIMUM SET. State which categories you will commit "
            "and which dimensions each owns. Each surviving category "
            "must add NEW signal — if a category is redundant with a "
            "stronger sibling and FRAMINGS gives no incremental "
            "reinforcement (or FACETS would double-fire on the same "
            "axis), drop it.\n"
            "\n"
    ...
    combine_mode: TraitCombineMode = Field(
        ...
        description=(
            ...
            "per-category scores into a single trait_score. Reads "
            "off the routing_exploration conclusion you committed "
            "just above — this field is the mechanical translation "
            "of that prose into the closed enum.\n"
            "\n"
            ...
            "alternative routes to the same signal.\n"
            "\n"
            ...
            "with one category, MAX and PRODUCT collapse to "
            "passthrough.\n"
            "\n"
            "NEVER:\n"
            "- DIVERGE FROM routing_exploration. If the exploration "
            "concluded FRAMINGS, this field is FRAMINGS. If you "
            "want a different answer, revise the exploration above.\n"
            "- DEFAULT-FILL. The mode is a real commit driven by "
            "the exploration's relationship analysis, not a "
            "placeholder."
        ),
    )
