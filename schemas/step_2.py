...
class Atom(BaseModel):
    ...
    evaluative_intent: str = Field(
        ...,
        description=(
            "A faithful restatement of what THIS criterion asks the "
            "search for, in 1-2 sentences. Content is bounded by "
            "surface_text plus the modifying_signals — every clause "
            "must trace back to a word in surface_text or an entry "
            "in modifying_signals.\n"
            "\n"
            "Inference is bounded to integrating each signal's effect "
            "mechanically: a hedge SOFTENS → preference-strength "
            "language; a polarity flip FLIPS POLARITY → avoid-"
            "direction language; a HARDENS modifier → required-"
            "strength language; freeform effect → restate the effect "
            "text. No content beyond what surface_text and the "
            "signals carry.\n"
            "\n"
            "NEVER:\n"
            ...
            "endpoint.\n"
            "- BROADEN, NARROW, OR INVENT against surface_text + "
            "modifying_signals. No exemplars, no canonical instances, "
            "no 'or'-clauses admitting neighboring criteria the user "
            "did not name, no sub-types from prior knowledge. If the "
            "query is concrete and modifying_signals is empty, this "
            "field stays close to a paraphrase of surface_text — "
            "that IS the correct shape, not a smell. Elaboration is "
            "licensed only by signal effects."
        ),
    )
