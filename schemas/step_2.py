...
class Trait(BaseModel):
    ...
    salience: Optional[Salience] = Field(
        ...
        description=(
            ...
            "logic.\n"
            "- HEDGES are the primary signal. Any hedge in "
            "modifiers ('ideally', 'preferably', 'a bit', 'kind "
            "of', 'maybe', 'would be nice') pushes the trait to "
            "SUPPORTING — the user took explicit language to "
            "mark it soft, and that intent is an incredibly "
            "strong signal that should not be overridden. Hedges "
            "win even when the trait is structurally prominent "
            "or named first.\n"
            "- NECESSITY language ('must', 'need', 'above all', "
            "'really') in modifiers or in purpose_in_query "
            "pushes toward CENTRAL when no hedge is present.\n"
            "- CORRECTIVE structures ('X but Y') often signal Y "
            "is central — the user is heading off a wrong "
            "assumption about a dimension they're tracking "
            "actively.\n"
            "- ORDER and POSITION are clues, not rules. Earlier-"
            "mentioned qualifiers and qualifiers in the adjective "
            "slot directly modifying the head noun lean CENTRAL "
            "when no hedge is present. These help when other "
            "signals are absent; they do not override hedges.\n"
            "\n"
            "EXAMPLES:\n"
            "- Trait 'creepy' (qualifier) in 'ideally creepy and "
            "atmospheric horror': SUPPORTING (the 'ideally' "
            "hedge is the primary signal; structural prominence "
            "does not override).\n"
            "- Trait 'not too long' (qualifier) in 'something "
            "funny for date night, not too long': SUPPORTING "
            "(hedge 'not too' plus trailing position).\n"
            "- Trait 'a bit dark' (qualifier) in 'a thriller, "
            "ideally a bit dark': SUPPORTING (multiple hedges).\n"
            "- Trait 'feel-good' (qualifier) in 'feel-good 90s "
            "comedy for date night': CENTRAL (no hedge; headline "
            "tonal want).\n"
            "- Trait 'slow-burn' (qualifier) in 'I really need a "
            "slow-burn thriller': CENTRAL (necessity language).\n"
