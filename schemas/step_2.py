...
class PossibleSplit(BaseModel):
    ...
    isolated_span: str = Field(
        ...
        description=(
            ...
            "- Parent 'lone female protagonist' → 'lone' and "
            "'female protagonist' (testing whether the structural "
            "single-lead idea and character-type idea should route "
            "separately).\n"
            "- Parent 'modern blockbuster' → 'modern' and "
            "'blockbuster' (testing whether the era and financial "
            "scale ideas each survive independently).\n"
    ...
    reasoning: str = Field(
        ...,
        description=(
            "PURPOSE: Decide whether the isolated_span should become "
            "its own trait. Pre-decision exploration — the "
            "should_split_out bool emerges from this reasoning, not "
            "the other way around. Do the analysis here, commit "
            "after.\n"
            "\n"
            "HOW TO THINK: Walk these four questions, in order:\n"
            "  1. Could this piece function as its own retrievable "
            "trait, either standalone or with the parent span's "
            "context preserved?\n"
            ...
            "\"'Lone' names a single-lead / solo-protagonist "
            "structure when read with the residual 'female "
            "protagonist'. Pulling it out preserves the intended "
            "meaning: one trait can search for the structural "
            "single-lead shape, while the residual searches for the "
            "female-protagonist character type. Keeping the full "
            "phrase whole would hide two independently routable "
            "requirements inside one category decision.\"\n"
            ...
            "\"'Modern' classifies cleanly as a release-era want. "
            "The residual 'blockbuster' also classifies cleanly as "
            "financial scale. Splitting preserves both constraints "
            "instead of forcing a single category to carry era and "
            "financial meaning at once.\"\n"
            ...
            "test — ignores the separate-routing criterion.)\n"
    ...
    should_split_out: bool = Field(
        ...
        description=(
            ...
            "piece works as a separately routed trait, has a category "
            "home, and pulling it out preserves meaning. False "
            "otherwise. Must be consistent with the prose — if the "
            "reasoning argues the piece can't route separately, this is "
...
class Decomposition(BaseModel):
    ...
    possible_splits: List[PossibleSplit] = Field(
        ...
        description=(
            ...
            "piece you considered isolating. Some stay absorbed; "
            "some split when they carry an independently routable "
            "meaning in context. Examples: 'lone female protagonist' "
            "tests 'lone'; 'modern blockbuster' tests 'modern'.\n"
...
class SpanAnalysis(BaseModel):
    ...
    text: str = Field(
        ...
        description=(
            ...
            "'preferably', 'must', 'above all', 'around', 'under', "
            "'at least', 'like', 'similar to', 'in the style of'. "
            "The span text holds the trait core; modifiers are "
            "tracked separately.\n"
            "\n"
            "EXAMPLES:\n"
    ...
    modifiers: List[str] = Field(
        ...
        description=(
            ...
            "'maybe', 'kind of'.\n"
            "- Necessity / emphasis markers: 'must', 'need', "
            "'above all', 'especially', 'really want'.\n"
            ...
            "- Range / approximation / ordinal calibration: "
            "'around', 'roughly', 'under', 'over', 'at least', "
            "'before', 'after', 'latest', 'most recent'.\n"
            "- Comparison / style scope: 'like', 'similar to', "
            "'in the vein of', 'in the style of', 'X-style'.\n"
            "\n"
            ...
            "- Span '90 minutes' in 'around 90 minutes' → "
            "['around'].\n"
            "- Span 'Inception' in 'movies like Inception' → "
            "['like'].\n"
...
class TraitModifier(BaseModel):
    ...
    text: str = Field(
        ...
        description=(
            ...
            "in', 'extremely', 'slightly', 'must', 'above all', "
            "'around', 'under', 'at least', 'like', 'similar to', "
            "'in the style of', 'based on'."
        ),
    )
    impact: str = Field(
        ...,
        description=(
            ...
            "actually said. Modifiers can weight, filter, bind a "
            "role, set a range, scope a comparison, or mark "
            "criticality — they rarely pin exact thresholds unless "
            "the wording itself does. Err toward "
            ...
            "satisfy this trait.\"\n"
            "- '90 minutes' + 'around': \"User wants runtime near "
            "this length, with some tolerance on either side rather "
            "than an exact duration.\"\n"
            "- 'movies' + 'at least three': \"User wants the count "
            "to meet or exceed three; fewer would not satisfy the "
            "request.\"\n"
            "- 'Inception' + 'like': \"Binds Inception as the "
            "comparison target — the user wants similar qualities, "
            "not literal title matching.\"\n"
            "- 'Stephen King' + 'based on': \"Binds Stephen King "
            "as the source-material creator rather than a film "
            "credit or stylistic comparison.\"\n"
            "\n"
            "INCORRECT EXAMPLES (avoid these patterns):\n"
...
class CategoryCandidate(BaseModel):
    ...
    fits: str = Field(
        ...
        description=(
            ...
            "\n"
            "CORRECT EXAMPLES:\n"
            "- Trait 'blockbuster', candidate Financial scale: "
            "\"Financial scale covers budget and box-office "
            ...
            "needed.\"\n"
            "- Trait 'blockbuster', candidate Cultural status / "
            "canonical stature: \"Cultural status covers whole-work "
            "reputation and public-position framings ('classic', "
            "'era-defining'). 'Blockbuster' carries an implied "
            "mainstream-status signal — a movie called blockbuster "
            "is widely recognized — but this is downstream of the "
            "financial signal. Plausible candidate; alignment is "
            "indirect rather than prototypical.\"\n"
            "- Trait 'creepy', candidate Emotional / experiential: "
            "\"Emotional / experiential covers tonal/experiential "
            ...
            "category.\"\n"
            "- Trait 'Tom Hanks', candidate Person credit: "
            "\"Person credit covers named-person constraints. "
            "'Tom Hanks' "
            "is a named actor — the category's home concept "
            "directly. Strong alignment.\"\n"
            "- Trait 'about AI', candidate Emotional / experiential: "
            "\"Emotional / experiential covers experiential framing. "
            "'About AI' is a topical-subject trait, which adjacent "
            "to but not the center of this category's axis. "
            "Plausible — "
            ...
            "- 'Financial scale is for financial scale and blockbuster "
    ...
    doesnt_fit: str = Field(
        ...
        description=(
            ...
            "without contrasting it with the trait. 'Cultural status "
            "is for reputation' is not a concern — it's a definition. "
            "The concern is 'this trait's reputation signal is "
            "downstream of its financial signal'.\n"
            "\n"
            "CORRECT EXAMPLES:\n"
            "- Trait 'blockbuster', candidate Cultural status / "
            "canonical stature: \"Cultural status captures "
            "whole-work reputation framing, which 'blockbuster' "
            "adjacent-implies but doesn't directly assert. The "
            "financial-scale axis is the dominant signal; "
            "reputation is a downstream consequence. Routing here "
            ...
            "- Trait 'classic', candidate Financial scale: "
            "\"Financial scale is about budget and box-office, but "
            "'classic' "
            "names canonical stature, not budget. The two "
            "are correlated — classic movies often had real "
            "budgets — but the trait's surface signal is cultural status, "
            "not financial. Strong concern: wrong axis entirely.\"\n"
            "- Trait 'about AI', candidate Emotional / experiential: "
            "\"'About AI' names a topical subject, while Emotional / "
            "experiential covers tonal/experiential framing. The "
            ...
            "topical.\"\n"
            "- Trait 'Tom Hanks', candidate Person credit: \"\" (empty — "
            "actor name maps cleanly, no boundary concern).\n"
            "- Trait 'creepy', candidate Emotional / experiential: \"\" (empty — "
            "trait is prototypical for this category).\n"
            "\n"
            "INCORRECT EXAMPLES (avoid these patterns):\n"
            "- 'Maybe doesn't fit.' (Phantom concern, no "
            "specifics. Should have been left empty.)\n"
            "- 'Cultural status is for reputation.' "
            "(Restates the category's purpose; doesn't say why this "
            "trait pulls away.)\n"
...
class Trait(BaseModel):
    ...
    query_phrase: str = Field(
        ...
        description=(
            ...
            "modifier.)\n"
            "- Query 'lone female protagonist' (decomposition split) "
            "→ trait 1: 'lone', trait 2: 'female protagonist'.\n"
    ...
    purpose_in_query: str = Field(
        ...
        description=(
            ...
            "long' / trait 'long': \"User wants the "
            ...
            "- 'Person-credit constraint, hard, central.' "
            ...
            "modifier nuance.)\n"
            "- 'This is a positive carver in Person credit with central "
    ...
    role: Role = Field(
        ...
        description=(
            ...
            "- First ask: what other trait does this one qualify? "
            "If there is no answer, this trait cannot be a "
            "qualifier; it defines the pool and is a CARVER.\n"
            "- CARVER if the trait stands on its own, or if "
            "purpose_in_query indicates a movie failing this trait "
            "should be eliminated rather than demoted. Hard "
            "requirements, definitional constraints, and named "
            "entities (actors, directors, franchises, genres, era "
            "ranges) are typically carvers.\n"
            "- QUALIFIER if purpose_in_query identifies the other "
            "trait or pool this trait is ranking, steering, or "
            "narrowing. Tonal preferences, fuzzy quality wants, and "
            "hedged language ('ideally', 'preferably', 'a bit') are "
            "typically qualifiers when they operate inside a pool "
            "defined by another trait.\n"
            "\n"
            ...
            "- 'feel-good film' (no other trait it qualifies): "
            "CARVER.\n"
            "- 'feel-good comedy' (feel-good qualifies comedy): "
            "QUALIFIER.\n"
            "- 'from the 90s' (era constraint, hard): CARVER.\n"
            "- 'a bit dark' (hedged tonal want): QUALIFIER.\n"
            "- 'about AI' (topical subject, hard ask): CARVER.\n"
            "- 'long' with modifier 'not too' in 'funny date-night "
            "movie, not too long' (runtime preference qualifying "
            "the date-night pool): QUALIFIER.\n"
    ...
    polarity: Polarity = Field(
        ...
        description=(
            ...
            "would have the compound modifier 'not too' and would "
            "be NEGATIVE because that phrase contains 'not')."
        ),
    )
    salience: Optional[Salience] = Field(
        ...
        description=(
            ...
            "or named first.\n"
            "- SUPPORTING-LANGUAGE cues in purpose_in_query "
            "('rounds out', 'sits in service of', 'marginal "
            "preference', 'supports the frame') push toward "
            "SUPPORTING when no hedge already settled the call.\n"
            "- NECESSITY language ('must', 'need', 'above all', "
            "'really') in modifiers or in purpose_in_query "
            "pushes toward CENTRAL when no SUPPORTING signal "
            "(hedge or supporting-language cue) is present.\n"
            ...
            "when no SUPPORTING signal is present. These help "
            "when SUPPORTING signals are absent; they do not "
            "override them.\n"
            "\n"
            "EXAMPLES:\n"
            ...
            "does not override).\n"
            "- Trait 'long' with modifier 'not too' (qualifier) in "
            "'something funny for date night, not too long': "
            "SUPPORTING (hedge 'not too' plus trailing position).\n"
