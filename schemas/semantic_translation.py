...
_CONTENT_DESC = (
    ...
    "a matching movie — match the verbosity, vocabulary, and "
    "register each sub-field uses on the ingest side. Per-sub-"
    "field descriptions on the matching Body class (PlotEventsBody "
    "/ ViewerExperienceBody / WatchContextBody / "
    "NarrativeTechniquesBody / PlotAnalysisBody / ProductionBody / "
    "ReceptionBody) carry the density, register, and synonym "
    "targets — read them before populating. Fold every aspect "
    "this space owns (per its SpaceCandidate.strengths) into ONE "
    "body; do not split one space's coverage across multiple "
    "entries.\n"
    "\n"
    "NEVER:\n"
    "- POPULATE A SUB-FIELD WITHOUT REAL SIGNAL. Empty sub-fields "
    "are valid and expected; padding dilutes the query vector. "
    "Default complement signals that the ingest side routinely "
    "emits — specifically ViewerExperienceBody negations naming a "
    "section's closest-opposite axis — DO count as real signal "
    "even when the user did not explicitly name a boundary; see "
    "ViewerExperienceBody field descriptions for the rule.\n"
    "- TRANSLATE WHEN THE REGISTER ALREADY MATCHES. Rewrite user "
    "phrasing into the space's ingest register only when they "
    "differ (e.g. 'it's so dumb but in a fun way' → "
    "tone_self_seriousness ['unselfserious', 'knowingly silly']). "
    "Keep user phrasing verbatim when it already lives in the "
    "ingest vocabulary — viewer_experience and watch_context use "
    "search-query vernacular, so 'tearjerker', 'edge of your "
    "seat', 'date night movie' should pass through, not get "
    "rewritten into critic-prose.\n"
    "- INVENT CONTENT BEYOND USER INTENT. Synonym expansion in "
    "term lists is allowed only for TRUE synonyms — substitutes "
    "the user would accept for their original phrasing without "
    "changing what they're asking for. Adjacent concepts that "
    "shift meaning ('haunting' → 'eerie' or 'supernatural'; "
    "'bittersweet' → 'tragic') change the retrieval target and "
    "are forbidden. In plot_events bodies, fabricating plot "
    "detail around a named motif ('clowns' → 'a clown chases a "
    "woman through a carnival') is the same failure mode — "
    "restate only what the user named.\n"
