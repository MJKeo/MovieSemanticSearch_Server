...
# Add parent directory to path to import from evaluations
sys.path.insert(0, str(Path(__file__).parent.parent))
from evaluations.eval import run_evaluation, EvaluationReport
...
def load_movie_id_to_title_map(movies_file: str | Path = None) -> dict[str, str]:
    ...
    # Create id -> title mapping using tmdb_id as the key
    id_to_title = {}
    for movie in movies:
        # Use tmdb_id (converted to string) as the key instead of IMDB id
        tmdb_id = movie.get('tmdb_id')
        title = movie.get('title', '')
        # Only add entries with both tmdb_id and title
        if tmdb_id is not None and title:
            id_to_title[str(tmdb_id)] = title
    
    return id_to_title


def calculate_best_worst_tables(
    report: EvaluationReport,
    movie_id_to_name_map: dict[str, str]
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Calculates best and worst performing movies for Overall, Content, and Vibe axes.
    
    For each axis, calculates the average of hits, rr, and ndcg metrics for each movie,
    then returns the top 5 (best) and bottom 5 (worst) performers.
    
    Args:
        report: The evaluation report containing per-movie results
        movie_id_to_name_map: Dictionary mapping movie ID to movie title
        
    Returns:
        Tuple of 6 DataFrames in order:
        (best_overall, best_content, best_vibe, worst_overall, worst_content, worst_vibe)
        Each DataFrame has columns: "Movie Title", "Hits@K", "MRR@K", "nDCG@K"
    """
    ...
    if not report or 'per_movie_results' not in report:
        return (empty_df, empty_df, empty_df, empty_df, empty_df, empty_df)
    
    per_movie_results = report['per_movie_results']
    
    # Process each axis: overall, content, vibe
    axes = ['overall', 'content', 'vibe']
    best_tables = []
    worst_tables = []
    ...
    for axis in axes:
        ...
        for movie_id, movie_result in per_movie_results.items():
            axis_result = movie_result.get(axis, {})
            ...
            avg_score = (hits + rr + ndcg) / 3.0
            
            # Get movie title
            movie_title = movie_id_to_name_map.get(movie_id, movie_id)
        ...
        movie_data_sorted_desc = sorted(movie_data, key=lambda x: x['avg_score'], reverse=True)
        movie_data_sorted_asc = sorted(movie_data, key=lambda x: x['avg_score'])
        
        # Get top 5 and bottom 5
        best_5 = movie_data_sorted_desc[:5]
        worst_5 = movie_data_sorted_asc[:5]
        
        # Create DataFrames
        best_df = pd.DataFrame({
            "Movie Title": [item['title'] for item in best_5],
            "Hits@K": [item['hits'] for item in best_5],
            "MRR@K": [item['rr'] for item in best_5],
            "nDCG@K": [item['ndcg'] for item in best_5]
        })
        
        worst_df = pd.DataFrame({
            "Movie Title": [item['title'] for item in worst_5],
            "Hits@K": [item['hits'] for item in worst_5],
            "MRR@K": [item['rr'] for item in worst_5],
            "nDCG@K": [item['ndcg'] for item in worst_5]
        })
        
        best_tables.append(best_df)
        worst_tables.append(worst_df)
    
    # Return in order: best_overall, best_content, best_vibe, worst_overall, worst_content, worst_vibe
    return (
        best_tables[0],  # best_overall
        best_tables[1],  # best_content
        best_tables[2],  # best_vibe
        worst_tables[0],  # worst_overall
        worst_tables[1],  # worst_content
        worst_tables[2]   # worst_vibe
    )
...
def run_evaluation_and_update(
    ...
    w_content: float,
    w_vibe: float
) -> Tuple[str, pd.DataFrame, str, pd.DataFrame, str, pd.DataFrame, str, pd.DataFrame, EvaluationReport, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    ...
    if not gt_path:
        gt_path = str(Path(__file__).parent / "most_similar_movies_v2.json")
    if not db_path:
        db_path = str(Path(__file__).parent.parent / "implementation" / "chroma_db")
    ...
    # Convert movie IDs to display titles for dropdown and sort alphabetically
    ...
    # Calculate best and worst performers tables
    best_overall_df, best_content_df, best_vibe_df, worst_overall_df, worst_content_df, worst_vibe_df = calculate_best_worst_tables(
        report, movie_id_to_name_map
    )
    
    # Return results for visualization plus the report for state, plus best/worst tables
...
def create_interface():
    ...
    """
    # Default paths (relative to evaluations/ directory)
    default_gt_path = str(Path(__file__).parent / "most_similar_movies_v2.json")
    default_db_path = str(Path(__file__).parent.parent / "implementation" / "chroma_db")
    ...
    with gr.Blocks(title="Movie Evaluation Visualization", css=custom_css) as interface:
        ...
        # Best and Worst Performers section
        gr.Markdown("---")
        gr.Markdown("## Best and Worst Performers")
        gr.Markdown("Top 5 and bottom 5 movies ranked by average metric score (Hits@K + MRR@K + nDCG@K) / 3")
        
        # First row: Best performers
        with gr.Row():
            gr.Markdown("### Best Performers")
        with gr.Row():
            with gr.Column():
                best_overall_table = gr.Dataframe(
                    label="Best Overall Scores",
                    headers=["Movie Title", "Hits@K", "MRR@K", "nDCG@K"],
                    interactive=False
                )
            with gr.Column():
                best_content_table = gr.Dataframe(
                    label="Best Content Scores",
                    headers=["Movie Title", "Hits@K", "MRR@K", "nDCG@K"],
                    interactive=False
                )
            with gr.Column():
                best_vibe_table = gr.Dataframe(
                    label="Best Vibe Scores",
                    headers=["Movie Title", "Hits@K", "MRR@K", "nDCG@K"],
                    interactive=False
                )
        ...
        with gr.Row():
            gr.Markdown("### Worst Performers")
        with gr.Row():
            with gr.Column():
                worst_overall_table = gr.Dataframe(
                    label="Worst Overall Scores",
                    headers=["Movie Title", "Hits@K", "MRR@K", "nDCG@K"],
                    interactive=False
                )
            with gr.Column():
                worst_content_table = gr.Dataframe(
                    label="Worst Content Scores",
                    headers=["Movie Title", "Hits@K", "MRR@K", "nDCG@K"],
                    interactive=False
                )
            with gr.Column():
                worst_vibe_table = gr.Dataframe(
                    label="Worst Vibe Scores",
                    headers=["Movie Title", "Hits@K", "MRR@K", "nDCG@K"],
                    interactive=False
                )
