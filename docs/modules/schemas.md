...
| `step_0_flow_routing.py` | **Search V2 Step 0** output schema. `Step0Response` with `titles_observed`, `qualifiers`, `ambiguous_title_phrases` (observation fields), plus `exact_title_flow`, `similarity_flow`, `standard_flow` decision fields, and `primary_flow` enum. Validators are **softening truncators** (not rejectors): `NONE_OF_THE_ABOVE` with stray entities → clears them; single-entry flows with N>1 entities → keeps first; list-style flows → unchanged. The entity-kind cross-check and qualifiers-force-no-entity validators were removed — the `to_*_flow_data` adapters handle reshaping without consulting candidate kinds. |
| `step_2.py` | **Search V2 Step 2 (Query Analysis)** output schema. `QueryAnalysis` with `intent_exploration: str` (replaces `holistic_read`), `atoms: list[Atom]`, and `traits: list[Trait]`. Each `Atom` carries `surface_text`, `modifying_signals: list[ModifyingSignal]`, `evaluative_intent: str`, `split_exploration`, and `standalone_check`. Each `Trait` carries `surface_text`, `evaluative_intent`, `qualifier_relation`, `anchor_reference`, `polarity`, `commitment_evidence`, `commitment` (required/elevated/neutral/supporting/diminished), `contextualized_phrase`, `relationship_role`, `replaces_axis`, `axes_replaced_by_siblings`. `ModifyingSignal` has `surface_phrase: str` and `effect: str`. |
| `step_3.py` | **Search V2 Step 3 (Trait Decomposition)** output schema (748 lines). `TraitDecomposition` couples three layers per trait: (1) trait-role analysis (`target_population` + `trait_role_analysis`), (2) a dimension inventory with per-dimension database-vocabulary routing candidates, and (3) `trait_restatement` + `combine_mode`. `category_candidates` carries a schema floor of `min_length=5`. Consumed by `search_v2/step_3.py` and Stage 4. |
| `implicit_expectations.py` | **Implicit-prior state schema** (220 lines). `Literal` axes (`PriorAxis`, `PriorDirection`, `PriorStrength`, `ExplicitCoverage`, `PriorRoom`, `TraitPressure`, `OrderingAxis`) describing inferred quality/popularity/ordering priors layered onto a query when not stated explicitly. |
| `consolidate_award_categories.py` | **Award category consolidation** (561 lines). Collapses the ~766 distinct raw `movie_awards.category` strings into 62 canonical concept slugs (e.g. all "best lead actor" variants → `lead-actor`). `norm()` normalizes punctuation/whitespace; used at ingest time so award queries match a stable vocabulary instead of free-form strings. |
| `trait_category.py` | **Category taxonomy for Step 2 and Step 3 grounding**. `CategoryName` enum — 43 active members (gaps at 43 and 45 preserved for cross-reference). Each member carries: `description`, `boundary`, `edge_cases`, `good_examples`, `bad_examples`, `endpoints` tuple, `bucket` (`HandlerBucket`), `combine_type`. Category definitions underwent an audit (clusters A1–A4, B1/B2/B3/B6): `SENSITIVE_CONTENT` is presence-only (never indexes absence); `MATURITY_RATING` allows suitability proxy use; `STORY_THEMATIC_ARCHETYPE` / `PLOT_EVENTS` / `CHARACTER_ARCHETYPE` / `ELEMENT_PRESENCE` have principle-based discriminators; geography/language categories each have "how to tell" tests. |
| `endpoint_parameters.py` | **Step 3 handler input types**. `EndpointParameters` abstract base + seven concrete per-endpoint wrapper classes (keyword / metadata / award / franchise / studio / entity / semantic). Each wrapper has a typed `parameters` field. `SemanticParameters` uses a single unified shape (not dual dealbreaker/preference classes) with `space_queries` list and retrospective `primary_vector` field. |
| `query_understanding.py` | **Legacy Stage 2A/2B** output schemas (pre-holistic-read redesign). `Step2AResponse` (concept extraction) + `QueryConcept` + `RetrievalExpression` + `Dealbreaker` / `Preference`. These classes are retained in file but `stage_2a.py` and `stage_2b.py` are deleted. Imports of `Step2AResponse` / `Step2BResponse` from elsewhere will fail. |
...
| `franchise_translation.py` | **Search V2 Step 3 franchise endpoint** output schema. `FranchiseQuerySpec` with `request_overview` (1-2 sentence interpretation field that every axis below cites as source-of-truth), `franchise_names` (list, OR-unioned across entries — covers umbrella-sweep alt forms and multi-franchise OR identically), `subgroup_names` (list, OR-unioned), `lineage_position` enum, `structural_flags` list (AND semantics), `launch_scope` enum, and `prefer_lineage` bool. Validator: dedups `structural_flags`; coerces `prefer_lineage` to False on no-name / multi-name / SPINOFF flag / subgroup-present; requires at least one axis populated (subgroup-only and structural-only specs are valid). |
| `metadata_translation.py` | **Search V2 Step 3 metadata endpoint** output schema. `MetadataTranslationOutput` with whole-call audit fields, explicit nullable sub-objects for the ten metadata attributes, and `scoring_method: ScoringMethod` for ANY/ALL multi-column composition. |
...
- `schemas/testing.ipynb` has been deleted — use the unit tests or a
  notebook under `search_v2/` for manual inspection.
...
- **`CategoryName` import path changed**: previously in `schemas.enums`, now
  in `schemas.trait_category`. Any `from schemas.enums import CategoryName`
  will fail. Use `from schemas.trait_category import CategoryName`.
- **`step_2.py` is `QueryAnalysis`, not the old multi-field Step 2 nor
  the holistic-only intermediate.** Top-level fields are `intent_exploration`
  + `atoms` + `traits` (`intent_exploration` replaced the old `holistic_read`).
  Each atom has `surface_text` + `modifying_signals` +
  `evaluative_intent` + `split_exploration` + `standalone_check`. The prior
  shape's `AbsorbedModifier` / `IncomingModification` / `ModificationDepth`
  / `AbsorbedModifierKind` types are deleted — `modifying_signals`
  collapses what they covered into one list. Any code importing the
  old types will fail.
- **`SemanticDealbreakerSpec` / `SemanticPreferenceSpec` are deleted.**
  Both collapsed into `SemanticParameters` in `semantic_translation.py`.
  Any imports of the old classes will fail.
