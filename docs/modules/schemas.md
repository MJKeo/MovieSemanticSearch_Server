...
- **`step_2.py` is `QueryAnalysis`, not the old multi-field Step 2 nor
  the holistic-only intermediate.** Top-level fields are `holistic_read`
  + `atoms`. Each atom has `surface_text` + `modifying_signals` +
  `evaluative_intent` + optional `candidate_internal_split`. The prior
