...
| `cost_tracking.py` | Request-scoped LLM + embedding cost accumulation. A `ContextVar`-held `RequestCostAccumulator` entered once per request via `track_request_cost()` (before the pipeline spawns tasks, so every branch inherits it by reference); `add_request_cost(cost_usd)` is called by each LLM provider (per billed attempt) and by `generate_vector_embedding`, and is a no-op outside a tracked request. The `/query_search` handler reads the total at stream end and writes it on the server span as `query_search.cost_usd`. Stdlib-only, mirroring `names.py`. |
