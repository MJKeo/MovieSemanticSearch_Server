...
## Category-level structured output for sparse multi-label LLM classification
**Observed:** When designing the concept tag output schema (27 binary tags, ~90% negative per movie), analyzed three approaches: boolean grid (27 bool fields), flat enum array, and category-level enum arrays. Boolean grids cause false-negative bias, positional fatigue, and autoregressive anchoring for sparse classification tasks. Flat arrays have recall failure (model stops emitting after 1-2 tags, skips entire categories). Category-level fields (7 groups of 1-8 tags each) force the model to consider each domain independently — each field transition acts as an attention redirect. Same mechanism as the 9-section narrative_techniques output. User approved the pattern.
**Proposed convention:** For LLM multi-label classification where positives are sparse (<20% of labels per item), use category-level array fields — not boolean grids or flat arrays. Each category field forces domain-specific consideration and provides a natural stopping/restarting point. Pair with evidence-before-answer field ordering (justification → label) for lightweight chain-of-thought.
**Sessions observed:** 1
