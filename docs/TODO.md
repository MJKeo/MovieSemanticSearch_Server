...
## Resolve default-MOVIE/theatrical behavior for media_type
**Context:** The deterministic `MEDIA_TYPE` router now constructs `MediaTypeEndpointParameters` from Step-3 category-call expressions without an LLM call, covering the three non-default `ReleaseFormat` values represented by the wrapper (`TV_MOVIE`, `SHORT`, `VIDEO`). Remaining design question: Step 3 can route phrases like "theatrical release" / "feature length" to `MEDIA_TYPE`, but the current wrapper intentionally excludes default `MOVIE`, so the router soft-fails those calls to empty. Decide whether theatrical/default-movie phrasing should stay unsupported, map to runtime/other categories upstream, or expand the media-type wrapper to represent `MOVIE`. The `exclude_shorts` discovery default also still wants resolving — likely as a deterministic rule around the same router.
**When:** Next time media-type routing behavior is tuned.
**See:** search_v2/stage_3/category_handlers/media_type_router.py; schemas/media_type_translation.py; search_v2/stage_3/media_type_query_execution.py; search_v2/stage_3/category_handlers/handler.py (MEDIA_TYPE short-circuit at Step 0a.2).
...
## Delete the deprecated `keyword_query_generation.py` standalone module
**Context:** `search_v2/endpoint_fetching/keyword_query_generation.py` was the original Python-built SYSTEM_PROMPT path for the keyword endpoint, before the category-handler/.md-chunk path took over. As of this session it has zero callers in production code (handler.py never imports it). Its sole valuable export — `_build_classification_registry_section()` and `_FAMILIES` — was lifted into `schemas/unified_classification_families.py`. Leaving the deprecated module alive risks someone importing the old `generate_keyword_query()` and getting a `ValidationError` against the new `KeywordQuerySpec` shape, plus the `_FAMILIES` data is now duplicated across two files (drift risk if one is edited).
**When:** When `unit_tests/test_keyword_query_generation.py` is also being touched in a test pass — the test imports `_build_classification_registry_section` from the deprecated module, so the test file needs to be either repointed at `schemas.unified_classification_families` or deleted alongside the module.
**See:** [search_v2/endpoint_fetching/keyword_query_generation.py](search_v2/endpoint_fetching/keyword_query_generation.py), [schemas/unified_classification_families.py](schemas/unified_classification_families.py), [unit_tests/test_keyword_query_generation.py](unit_tests/test_keyword_query_generation.py)

## Refresh `unit_tests/test_prompt_builder_system_prompt.py` for substitution
**Context:** This session added per-route `{{TOKEN}}` substitution to `prompt_builder._ENDPOINT_CHUNKS` at module import time. Endpoint chunks that previously contained literal `{{...}}` text now expand to multi-thousand-char rendered registries (keyword grew from ~2.5k to ~41k chars, awards picked up three rendered sections, studio gained the brand registry). Any test in this file that snapshots prompt content, asserts on `_ENDPOINT_CHUNKS[route]` size, or expects literal `{{TOKEN}}` substrings will break — they need updating. Also worth adding tests that exercise the two new safety nets: (a) registered token missing from .md raises; (b) leftover `{{...}}` after substitution raises. I did not run the test suite per the test-boundaries rule.
**When:** Next time test work touches search_v2/category_handlers, or in a dedicated test pass after this session's changes.
**See:** [unit_tests/test_prompt_builder_system_prompt.py](unit_tests/test_prompt_builder_system_prompt.py), [search_v2/endpoint_fetching/category_handlers/prompt_builder.py](search_v2/endpoint_fetching/category_handlers/prompt_builder.py)

## Update `search_method_deterministic_logic.md` for Stage 4 implementation deltas
**Context:** Stage 4 (`search_v2/stage_4_execution.py`) implemented two design choices that diverge from how `search_improvement_planning/search_method_deterministic_logic.md` currently reads: (1) trait-level composite uses **nested equal-weight averaging** (per-category composite, then per-trait average across categories) — §6 currently writes the trait composite as a single flat average across all calls in the trait; (2) negative-polarity traits now use a **three-bin `gate × fuzzy` formula** — gate is `∏ G_a` over would-be-CANDIDATE_GENERATOR calls whose category is in `_AUTHORITATIVE_NEGATION_CATEGORIES` (specific-entity / structured-metadata), fuzzy is `1 − ∏(1 − s)` (noisy-OR) over evidential-G plus would-be-POOL_RERANKER calls. Would-be type is re-derived via `determine_operation_type(category, route, Polarity.POSITIVE)` because `spec.operation_type` is uniformly `POOL_RERANKER` for negatives. §6 / §8 currently describe negative-trait scoring as the same equal-weight-sum shape as positive traits with a sign flip applied at §9. The negative-trait formula has gone through one revision since the original Stage 4 landing — it was previously `(∏ G) × mean(R)` (gated multiplicative-AND) but failed on fuzzy negative concepts ("not scary" decomposed as KEYWORD:horror + SEMANTIC:scary, where the keyword tag has low recall and gating R behind it under-penalized scary thrillers).
**When:** When next reviewing the planning doc, or before any future Stage-4 design refresh. The implementation behaves as the user explicitly chose; the doc is the artifact that needs to catch up.
**See:** search_v2/stage_4_execution.py (`_run_cand_gen_trait` for nested averaging; `_score_negative_trait` for gate × fuzzy + the `_AUTHORITATIVE_NEGATION_CATEGORIES` frozenset), docs/modules/search_v2.md §"Stage 4 — Execution & Ranking" (already documents the current divergence)

## Unit tests for `search_v2/stage_4_execution.py`
**Context:** New module landed without test coverage per .claude/rules/test-boundaries.md. Worth covering eventually — these are the highest-leverage cases: (a) nested averaging composes correctly across category → trait → branch (synthetic 2-trait, 2-category, 2-call setup with known scores); (b) negative-trait three-bin `gate × fuzzy` formula — (b1) authoritative-G gate ANDs definitively (1×1×R for full-match, 0×_ for partial-mismatch); (b2) evidential-G + R noisy-OR reinforces (two 0.5 fuzzy calls saturate to 0.75); (b3) failed calls (None) are dropped from their bin rather than zeroing the gate or saturating the OR; (b4) `_AUTHORITATIVE_NEGATION_CATEGORIES` membership routes correctly (PERSON_CREDIT to G_a, GENRE to fuzzy); (b5) `ScoreBreakdown.positive_total + negative_total` equals the sorted `ranked` score for every movie; (c) rarity tier boundaries on a small synthetic corpus (~150 movies) exercising each tier; (d) auxiliary shorts subtraction runs after cand-gen union and before rerankers; (e) NEUTRAL_SEED additive seeding fires only when branch_pool empty AND spec is present; (f) branch with `branch_error` set yields empty `ranked` and propagates the error string; (g) cand-gen trait with empty pool drops silently (returns None from `_run_cand_gen_trait`); (h) opportunity-cost rule — cand-gen trait contributes 0 to candidates it didn't generate, but its weight still counts in the §9 sum.
**When:** Next dedicated testing pass on search_v2.
**See:** search_v2/stage_4_execution.py, search_improvement_planning/search_method_deterministic_logic.md (canonical scoring spec)

## Reconcile MEDIA_TYPE shorts handling — soft-scoring vs hard-exclusion paths
**Context:** The pipeline has two paths for handling shorts that produce different outcomes: (1) when the user does NOT mention shorts, `_build_auxiliary_specs` injects a `MEDIA_TYPE` shorts-exclusion auxiliary spec that hard-removes SHORT-format movies from `branch_pool` via set difference in `_apply_auxiliary` — full categorical exclusion. (2) when the user explicitly says "not a short film" (or similar), it produces a negative trait with `CategoryName.MEDIA_TYPE`, which goes through normal negative-trait scoring (`_score_negative_trait` puts MEDIA_TYPE in the authoritative-G bin per `_AUTHORITATIVE_NEGATION_CATEGORIES`). That path soft-penalizes shorts via `gate × fuzzy` and the §9 sign flip — does NOT remove them from the pool. So the more explicit user instruction ("not a short film") produces the LESS strict behavior. The pre-existing comment in `_build_shorts_exclusion_spec` already names shorts as the only true categorical exclusion in the pipeline ("everything is soft scoring — no trait ever filters categorically"), so the soft-scoring path for explicit-no-shorts is consistent with that design rule, just surprising. Two reasonable resolutions: (a) when an explicit MEDIA_TYPE negative trait targets SHORT, escalate it to the auxiliary hard-exclusion path so the explicit instruction is at least as strict as the implicit default; (b) document the asymmetry as deliberate and accept the soft-penalty behavior for explicit cases. Pre-existing — not introduced by the negative-trait formula fix in this session, but surfaced during code review of that fix.
**When:** When next touching `_build_auxiliary_specs` / `_apply_auxiliary` or the negative-trait scoring path. Low priority — explicit-no-shorts queries are uncommon and the soft penalty still ranks shorts last in practice.
**See:** search_v2/full_pipeline_orchestrator.py `_build_shorts_exclusion_spec` / `_has_media_type_call` / `_build_auxiliary_specs` (lines 471-518), search_v2/stage_4_execution.py `_apply_auxiliary` (lines 791-866) and `_score_negative_trait` (lines 692-784)

## Test coverage for ISO-code mappings on Country / Language enums
**Context:** This session added `iso_code` (ISO 3166-1 alpha-2 / ISO 639-1) and a `from_iso()` classmethod to both `Country` (262 members, 249 with codes) and `Language` (334 members, 140 with codes), plus `field_validator`s on `AudioLanguageTranslation` / `CountryOfOriginTranslation` that resolve ISO codes to enum members. No unit tests were authored per the test-boundaries rule. Coverage gaps that should be exercised: (a) every member with a non-None `iso_code` round-trips through `from_iso()`; (b) the reverse-lookup dict rejects duplicates at import (already asserted in code, but no test pins the contract); (c) the schema-side validators reject unknown 2-letter codes (`xx` / `XX`), pattern-fail on wrong case (`EN` / `us`), and pattern-fail on 3+ letter inputs (`eng` / `USA`); (d) the executor handlers in `metadata_query_execution.py` produce the correct `language_id` / `country_id` arrays after parsing.
**When:** Next test-suite pass touching `unit_tests/test_languages.py` or any Stage-3 metadata test.
**See:** implementation/classes/countries.py, implementation/classes/languages.py, schemas/metadata_translation.py, search_v2/endpoint_fetching/metadata_query_execution.py, DIFF_CONTEXT.md "ISO codes for Country/Language in metadata LLM schema"

## Reduce UnifiedClassification × 2 reference cost in keyword subintent schema
**Context:** During the OpenAI 1000-enum-cap diagnosis, the flat-enum analysis showed `UnifiedClassification` (260 members) is referenced TWICE inside the keyword subintent wrapper — once on `KeywordQuerySpecSubintent.finalized_keywords[items]` (commit layer) and once on `PotentialKeyword.keyword` (analysis layer per shortlisted candidate). The duplication is by design — the analyze-then-commit scaffold anchors each candidate to a registry member with coverage-rationale prose — but it costs 520 of the schema's enum-value budget. The user explicitly punted ("Ignore") because the ISO-code change for Country/Language already cleared 596 from the budget and brought TARGET_AUDIENCE / SENSITIVE_CONTENT to ~597 (well under the 1000 cap). If a future Bucket-8 category is added that brings additional enum surface (or `UnifiedClassification` grows past ~370 members), this is the next lever — demote `PotentialKeyword.keyword` from the enum to `str` with a description naming the registry, and validate the candidate is a real member post-parse. `finalized_keywords` already enum-validates the committed list, so analysis-layer schema-time validation isn't load-bearing.
**When:** Only if another Bucket-8 category pushes a schema close to or over the 1000-enum cap, or if `UnifiedClassification` membership grows substantially.
**See:** schemas/keyword_translation.py:39-115 (`PotentialKeyword.keyword` and `KeywordQuerySpec[Subintent].finalized_keywords`), DIFF_CONTEXT.md "ISO codes for Country/Language in metadata LLM schema"

## Per-category schema enum-count guardrail
**Context:** This session diagnosed that `TARGET_AUDIENCE` / `SENSITIVE_CONTENT` had silently exceeded OpenAI's 1000-enum-per-schema cap when bucket-8 categories combined three endpoint subintent wrappers. The breakage didn't surface until an actual handler call hit a 400 from OpenAI. The flat-enum-count algorithm (sum-of-`$ref`-occurrences-times-enum-size + inline enum literals) is small (~15 lines) and was written twice from scratch in this session for diagnosis and verification. A regression script that imports every category schema, runs the flatten algorithm, and asserts each is < 1000 would catch any future schema change that adds an enum-bearing field to a multi-endpoint bucket before it ships.
**When:** Anytime — low effort, prevents recurrence of the same silent failure mode. Could live as a pytest test or as a standalone script in `search_v2/endpoint_fetching/category_handlers/` (e.g., `audit_schema_enum_counts.py`).
**See:** schemas/metadata_translation.py (the fix from this session), DIFF_CONTEXT.md "ISO codes for Country/Language in metadata LLM schema" (verification block in the original entry has the flatten algorithm)

## ✅ RESOLVED 2026-05-07 — Auteur list composition for similar-movies V3 director lane
**Context:** The V3 similar-movies director-lane rework replaces the V2 `mv_director_strength` percentile-based boost with a **manual auteur list**. Composition was deferred from the planning session to keep the list curated rather than performative.
**Resolution:** 60-entry list finalized and embedded in [search_improvement_planning/similar_movies_v3_plan.md](../search_improvement_planning/similar_movies_v3_plan.md) §2.1, with normalized keys verified against the live `lex.lexical_dictionary` and `lex.inv_director_postings`. Decisions resolved: (a) inclusion criteria — three-criterion bar (recognizable style across ≥3 features, general moviegoer awareness, broader catalog likely to "feel the same"); franchise-defining genre directors only when the broader catalog also coheres (Carpenter ✅; "Star Wars creator George Lucas" ❌). (b) Sourcing — hand-picked from parametric knowledge plus a research subagent's auteurism survey, then filtered against the bar. (c) Data-side — Coen brothers stored as **two separate keys** (`joel coen`, `ethan coen`); lane fires for either credit independently. (d) Storage — flat boolean for now (auteur or not); tiering can be revisited if the additive `0.20` weight turns out coarse.
**Three credit-string corrections** (verified by direct DB query): Bong Joon-ho stored as `bong joon ho` (no hyphen); Hirokazu Kore-eda as `hirokazu koreeda` (no hyphen, no space); Alejandro González Iñárritu as `alejandro g inarritu` (period collapses to "g", diacritic stripped).
**Held back pending separate user decision (not blocking):** Woody Allen, Mel Gibson, Roman Polanski — stylistic signal is real but the inclusion call ties to public-discourse considerations rather than craft.
**See:** search_improvement_planning/similar_movies_v3_plan.md §2.1 (full table), §6 #1 (resolution log)

## V3 similar-movies — calibrate rare-keyword IDF tier thresholds
**Context:** V3 §2.6 rare-keyword lane uses tier thresholds 2.5 (low/moderate) and 4.5 (moderate/high), with floor triggers at single-trait IDF ≥5.0 OR sum-of-moderate-and-high ≥7.0 (see RARE_KW_* constants in [search_v2/similar_movies.py](../search_v2/similar_movies.py)). Verification pass against the live catalog showed actual IDF distribution is gentler than the v3 plan assumed: TMDB genres all sit at IDF <1.0 (max ~1.0), and the floor rarely activates in the 21+13 smoke runs. The tiered additive contribution still works (Oppenheimer top 1 has rare_keyword > 0), but the floor mechanism designed to override shape on rare-combo hits is largely dormant.
**When:** When a real failure case shows the floor *should* have fired but didnt — i.e., a candidate with multiple distinctive shared tags that doesnt make the top 10 because shape drift held it back. Lowering thresholds (e.g., LOW_MAX 1.5, MODERATE_MAX 3.0, FLOOR_HIGH_SINGLE 3.5, FLOOR_COMBO_SUM 5.0) would let the floor mechanism fire on real distinctive-match cases without changing the additive contribution semantics.
**See:** search_v2/similar_movies.py RARE_KW_* constants; /tmp/v3_verification_report.md follow-up #1.

## V3 similar-movies — verify Nolan director_signature fires on all his films
**Context:** Inception single-anchor top 1 is Tenet (high shape match, 0.822 score) but the director lane only fires director_signature on The Prestige (#6) and Memento (#9), not Tenet/Trance. Nolan IS in the curated auteur list (`christopher nolan` in [search_v2/auteur_directors.py](../search_v2/auteur_directors.py)), so the lane SHOULD be firing on all Nolan candidates. Possible explanation: the candidate-generation lane (`fetch_director_movie_terms`) might not be returning all Nolan films for the Inception anchor, OR the per-candidate director set is being computed differently than the auteur lookup expects.
**When:** Spot-check before declaring V3 director rework complete. Run a targeted query: for anchor=Inception and director term_id=normalize_string("christopher nolan"), confirm `director_candidate_terms` returns Tenet/Trance/Memento/Prestige/Dunkirk/etc. — every Nolan film in the catalog. If the candidate lane is selective, the issue is upstream of the auteur scoring.
**See:** /tmp/v3_verification_report.md follow-up #2; search_v2/similar_movies.py _single_anchor_director_score.

## Step 1 — multi-anchor specific queries still anchor-lock on the concept combination
**Context:** Step 1's prompt was rewritten this session to produce alternative search "spins" that diverge from the original query. Single-anchor specific queries (Tom Hanks, Scorsese, Pixar) now drop their named anchor and build genuine sibling searches. Multi-anchor composite queries — "sci-fi adventures with a strong mentorship arc", "John Wick but woman", "90s romantic comedies set in new york" — still produce spins that keep both core anchors because the model reads the combination AS the viewer's taste rather than as the surface query. To break this, the model would need to lift the abstraction one level — recognize that "sci-fi mentorship" is an instance of "hero's journey / passing the torch" and produce trans-genre siblings (kung-fu master/student films, war veteran-rookie pairings, sports coaching films). The model doesn't make this lift spontaneously and the current prompt doesn't push it. User accepted this ceiling for now; revisit if it becomes a real-traffic pain point.
**When:** When real-traffic data shows multi-anchor specific queries are a meaningful share of search volume, or when an evaluation surfaces the anchor-lock as a measurable quality regression.
**See:** search_v2/step_1.py (HOW_TO_THINK block), search_v2/run_step_1.py, DIFF_CONTEXT.md Step 1 entry (final iteration round paragraph 6 + end-of-session ceilings paragraph).

## Step 1 — evaluative-language leak in spin queries and ui_labels
**Context:** Step 1's avoid list explicitly bans evaluative quality language ("iconic", "highly-rated", "prestigious", "stunning", "heartfelt", "magical", "classics") in spins, framed as principle: "words that ask the search for prestige, popularity, or judgment rather than describe what kind of movie the user wants." After 4 iterations of prompt refinement, these terms still appear in some spin queries and labels — especially in `ui_label` slots where compression pressure pulls the model toward punchy adjectives ("Cozy British Classics", "Inspiring Life Journeys", "Masterful Grifters"). Principle-based reframe didn't fully suppress the leak. Possible next moves: (a) positive reframe (tell the model what TO describe instead of what to avoid); (b) tighten the schema description for `ui_label` with a category-of-words constraint; (c) Pydantic validator that flags evaluative adjectives at parse time. Not addressed this session per user direction.
**When:** Next time Step 1 prompts are revisited, or if downstream search-quality eval shows evaluative-label-driven retrieval issues.
**See:** search_v2/step_1.py (HOW_TO_THINK Avoid list, item 3), schemas/step_1.py (Spin.ui_label description), DIFF_CONTEXT.md Step 1 entry (end-of-session ceilings paragraph).
