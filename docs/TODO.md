...
## 🛑 ESSENTIAL — Auteur list composition for similar-movies V3 director lane
**Context:** The V3 similar-movies director-lane rework (see [search_improvement_planning/similar_movies_v3_plan.md](../search_improvement_planning/similar_movies_v3_plan.md) §2.1) replaces the V2 `mv_director_strength` percentile-based boost with a **manual auteur list**. Directors on the list get the full director-lane boost in single-anchor flow; directors not on the list get zero director signal in single-anchor (multi-anchor still fires on any repeated director regardless of auteur status). The list is the load-bearing artifact of the rework — getting it wrong silently degrades single-anchor results across the catalog. Composition was deferred to a follow-up conversation rather than guessed in-session, to keep the list curated rather than performative.
**Decisions still to make:**
- Inclusion criteria: pure stylistic auteurs only (Anderson, Lynch, Tarantino, Coens), or include franchise-defining genre directors whose style is recognizable within their genre (Carpenter, Romero, Cronenberg)?
- Sourcing: hand-pick by cinephile sense, seed from external rank (Sight & Sound 2022 Directors poll, Letterboxd top-rated), or hybrid?
- Data-side resolution per director: are Coen Brothers one director term ID in our data or two? Cronenberg-the-elder vs Cronenberg-the-younger?
- Storage format: flat boolean (auteur / not) or tiered (high-auteur / mid-auteur / not)?
**🛑 HARD PAUSE RULE:** If implementation work on the V3 director lane begins before this list is finalized, **STOP and surface the gap to the user before proceeding.** Do not stub the list with a placeholder, do not fall back to the V2 `mv_director_strength` MV, and do not guess at composition based on training knowledge. Any code path depending on the auteur list must remain unimplemented until the list is locked.
**When:** Before any work on V3 §2.1 (director rework) starts. Independent of the V3 plan items 4 (themes lane) and 5 (country/language) which can ship without the list.
**See:** search_improvement_planning/similar_movies_v3_plan.md §2.1 (the rework that depends on this), §6 #1 (open question), priority-table item 7 (BLOCKED until resolved)
