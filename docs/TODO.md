...
## Wire `release_format` into the search side
**Context:** The ingestion-side groundwork landed this session: `ReleaseFormat` enum (UNKNOWN=0, MOVIE=1, TV_MOVIE=2, SHORT=3, VIDEO=4) on `schemas/enums.py`, `release_format SMALLINT NOT NULL DEFAULT 0` column on `movie_card`, `upsert_movie_card` populates it, and the backfill (`movie_ingestion/backfill/backfill_release_format.py`) seeded the existing 100K+ rows. Search-side use is explicitly out of scope and deferred. The remaining work, per the architecture decided in this session: (a) add a `RELEASE_FORMAT` value to `CategoryName` in `schemas/enums.py` (the existing tuple-constructor pattern with description + endpoints) so Step 2 can categorize fragments like "shorts" / "Pixar shorts" / "direct-to-video" / "TV movies" against it; (b) add three release-format candidate-generating endpoints (`type:short`, `type:video`, `type:tvMovie`) that load the membership set at startup from `movie_card` and emit flat-score candidates supporting both inclusion and exclusion modes; (c) derive `exclude_shorts` deterministically in code from Step 2's output (no new LLM call) — set true by default, flip to false whenever Step 2 emits a non-negated `RELEASE_FORMAT=SHORT` atom; (d) thread `exclude_shorts` through the rest of the endpoints so shorts get filtered from broad discovery results unless the user explicitly asked for them.
**When:** Next time search-side work picks up — sequence after the current category-handler / Step-2 vocabulary changes settle.
**See:** DIFF_CONTEXT.md "release_format column on movie_card (ingestion side only)" entry; ~/.claude/plans/for-now-let-s-just-mighty-diffie.md (the ingestion plan, with its "explicitly out of scope" section enumerating what this TODO covers); schemas/enums.py (ReleaseFormat enum, CategoryName for the new entry), movie_ingestion/backfill/backfill_release_format.py.
