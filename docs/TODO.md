...
**Context:** Stage 4 (`search_v2/stage_4_execution.py`) implemented two design choices that diverge from how `search_improvement_planning/search_method_deterministic_logic.md` currently reads: (1) trait-level composite uses **nested equal-weight averaging** (per-category composite, then per-trait average across categories) — §6 currently writes the trait composite as a single flat average across all calls in the trait; (2) negative-polarity traits use **multiplicative-AND on would-be-generator calls × mean of would-be-reranker calls** (gated multiplication), with would-be-type re-derived via `determine_operation_type(category, route, Polarity.POSITIVE)` because `spec.operation_type` is uniformly `POOL_RERANKER` for negatives — §6 / §8 currently describe negative-trait scoring as the same equal-weight-sum shape as positive traits with a sign flip applied at §9.
...
**See:** search_v2/stage_4_execution.py (`_run_cand_gen_trait` for nested averaging; `_score_negative_trait` for gated multiplication), DIFF_CONTEXT.md §"Stage-4 execution + ranking layer", docs/modules/search_v2.md §"Stage 4 — Execution & Ranking" (already documents the divergence)

## Unit tests for `search_v2/stage_4_execution.py`
**Context:** New module landed without test coverage per .claude/rules/test-boundaries.md. Worth covering eventually — these are the highest-leverage cases: (a) nested averaging composes correctly across category → trait → branch (synthetic 2-trait, 2-category, 2-call setup with known scores); (b) negative-trait gated-multiplicative shape — a candidate matching all G calls × non-zero R mean produces a meaningful penalty; matching only some G calls drops the penalty steeply; failed calls (None) are dropped from both partitions rather than zeroing the product; (c) rarity tier boundaries on a small synthetic corpus (~150 movies) exercising each tier; (d) auxiliary shorts subtraction runs after cand-gen union and before rerankers; (e) NEUTRAL_SEED additive seeding fires only when branch_pool empty AND spec is present; (f) branch with `branch_error` set yields empty `ranked` and propagates the error string; (g) cand-gen trait with empty pool drops silently (returns None from `_run_cand_gen_trait`); (h) opportunity-cost rule — cand-gen trait contributes 0 to candidates it didn't generate, but its weight still counts in the §9 sum.
