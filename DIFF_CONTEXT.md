...
## Deduplicate awards by best outcome before Postgres insertion
Files: movie_ingestion/final_ingestion/ingest_movie.py | IMDB can return multiple award entries for the same (ceremony, award_name, category, year) when a movie has multiple nominees in the same category. Since we don't store nominee names, these cause PK violations during ingestion. Added dedup in `ingest_movie_awards()` that groups by PK fields and keeps only the entry with the lowest `outcome_id` (best outcome).
