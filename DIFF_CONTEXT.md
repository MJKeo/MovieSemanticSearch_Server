...
## Cat 44 (CHRONOLOGICAL) — bespoke schema + bucket move
Files: schemas/chronological_translation.py, schemas/enums.py, schemas/trait_category.py, search_v2/endpoint_fetching/category_handlers/endpoint_registry.py, search_v2/endpoint_fetching/category_handlers/handler.py, search_v2/stage_4_execution.py, search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/chronological.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/chronological.md, search_improvement_planning/query_categories.md
Why: CHRONOLOGICAL was routed through MetadataEndpointParameters' full ~10-field schema even though it only expresses sort-direction + optional count. The breadth fed the LLM's non-null bias — bare ordinals like "newest Bond" produced fabricated top_n values, contaminating downstream selection.
Approach: New EndpointRoute.CHRONOLOGICAL with a bespoke `ChronologicalQuerySpec(EndpointParameters)` — three fields: `direction` (ChronologicalDirection enum: chronological / reverse_chronological), `is_top_n_active` (explicit gate so the LLM cannot fill top_n without committing to having read a count from the user), `top_n` (int, intentionally unbounded per user instruction). Moved CHRONOLOGICAL out of SINGLE_METADATA_ENDPOINT into SINGLE_NON_METADATA_ENDPOINT so the bucket prompt no longer pitches typed-value / range / threshold framings irrelevant to ordinal pick. handler.determine_operation_type now branches on the new route directly for POOL_RERANKER (CHRONOLOGICAL removed from the METADATA branch's special-case tuple). Prompt + few-shot files fully rewritten to describe field-level emit semantics with concrete value assignments per example.
Design context: Pattern mirrors PersonQuerySpec (schemas/entity_translation.py:190) — direct EndpointParameters subclass with no nested parameters wrapper. ChronologicalDirection lives co-located in chronological_translation.py (not enums.py) because it has no other consumer. stage_4_execution.py:1066 negation-set comment updated to point at the new route; the logic (POOL_RERANKER → excluded from G_a partition) is unchanged.
Testing notes: Need executor-side wiring before this is end-to-end functional — ChronologicalQuerySpec instances will flow into HandlerResult.preference_specs but no downstream executor reads them yet (the previous METADATA path executed via the metadata endpoint executor). Verify via test_v3_endpoints.ipynb that (a) the four prompt examples produce valid field assignments, (b) "newest Bond" / "latest Marvel" leave is_top_n_active=false, (c) "three earliest Kubrick" sets is_top_n_active=true with top_n=3, (d) "recent movies" no-fires. determine_operation_type(CHRONOLOGICAL, CHRONOLOGICAL, POSITIVE) should return POOL_RERANKER. get_output_wrapper(CHRONOLOGICAL, SINGLE_NON_METADATA_ENDPOINT) should return ChronologicalQuerySpec.

## Re-route NAMED_SOURCE_CREATOR from SEMANTIC to ENTITY (writer-credit lookup)
Files: schemas/trait_category.py, schemas/entity_translation.py, search_v2/endpoint_fetching/category_handlers/endpoint_registry.py, search_v2/full_pipeline_orchestrator.py, search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/named_source_creator.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/named_source_creator.md
Why: Database audit (74/78 canonical adaptations across King/Tolkien/Shakespeare/PKD/Gaiman/Austen/Christie — 94.9% recall, ~100% within authors with distinctive names) showed the IMDB writer credit field is the authoritative carrier for source-author membership. SEMANTIC routing was unreliable: author content signatures are inconsistent across an author's oeuvre (King spans horror/drama/coming-of-age; Shakespeare modernizations bury the source connection entirely), and the only reliable vector-text leak ("Stephen King" mentioned in reception_summary) only fires for ~50% of films and is a brittle accident of generation prompts, not a designed signal. The 5% miss (Lion King, She's the Man, West Side Story, Clueless) is the "inspired by" class — uncredited at IMDB AND undetectable semantically — so no current channel can retrieve those anyway.
Approach: New `WriterOnlyPersonQuerySpec(PersonQuerySpec)` in schemas/entity_translation.py with a model_validator(mode="after") that REVISES rather than REJECTS — coerces `person_category=WRITER` and `prominence_mode=DEFAULT` on every target post-parse. Reuses the full PersonQuerySpec shape so the LLM still does the alias-enumeration walk (J.R.R. Tolkien vs John Ronald Reuel Tolkien, Philip K. Dick vs Philip Kindred Dick); only the role pick is hard-wired. Registered in _ENTITY_DISPATCH so endpoint_registry.get_output_wrapper hands the LLM this tighter spec; trait_category.py NAMED_SOURCE_CREATOR routing changed from (SEMANTIC,) to (ENTITY,). Stale `NAMED_SOURCE_CREATOR: CONCRETE_FACT_OR_IDENTIFIER` entry dropped from full_pipeline_orchestrator._SEMANTIC_PROMOTION_TIERS — the map only fires when route is SEMANTIC, which this category no longer is.
Design context: Mirrors the existing per-category dispatch pattern (PERSON_CREDIT → PersonQuerySpec, NAMED_CHARACTER → CharacterQuerySpec). Scoring is binary row-presence via entity_query_execution._fetch_binary_role_scores — prominence_mode is read off but ignored for non-actor roles (see comment at entity_query_execution.py:412), so the validator's coercion is a normalization, not a behavior change for the executor. NAMED_SOURCE_CREATOR was already in stage_4_execution._AUTHORITATIVE_NEGATION_CATEGORIES (definitional membership), which is now actually accurate.
Testing notes: Verify (a) WriterOnlyPersonQuerySpec parses an LLM payload with `person_category="director"` and silently rewrites it to WRITER without raising, (b) get_output_wrapper(ENTITY, SINGLE_NON_METADATA_ENDPOINT, category=NAMED_SOURCE_CREATOR) returns WriterOnlyPersonQuerySpec, (c) end-to-end "Stephen King movies" produces a writer-table lookup with forms=["Stephen King"] and retrieves the 261 King-credited films in our DB, (d) "Tolkien adaptations" emits both "J.R.R. Tolkien" and the expanded form so films credited under the long form are not silently dropped, (e) prompt assembly via build_system_prompt(NAMED_SOURCE_CREATOR) succeeds and contains the new writer-fixed guidance (does not regress the preexisting chronological.md missing-file error which is unrelated).

## Cat 44 (CHRONOLOGICAL) — schema trim + executor + endpoint prompt
Files: schemas/chronological_translation.py, schemas/trait_category.py, db/chronological_scoring.py, search_v2/endpoint_fetching/chronological_query_execution.py, search_v2/endpoint_fetching/endpoint_executors.py, search_v2/endpoint_fetching/category_handlers/prompts/endpoints/chronological.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/chronological.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/chronological.md, search_improvement_planning/query_categories.md

### Intent
Convert chronological-ordinal from a top-N pick (ordinal selection) into a continuous percentile-rank curve over the candidate pool's release_ts. User explicitly preferred treating chronological like any other qualifier trait: the most-extreme movie in the requested direction always wins, every distinct date matters, and the score blends into the multi-channel rerank rather than carving a hard top-N slice. Distinct from RELEASE_DATE (Cat 13): that endpoint emits a saturated range/window curve (everything inside the window scores ~1); this endpoint never saturates.

### Key Decisions
- **Schema trimmed to a single `direction` field.** Removed `is_top_n_active` and `top_n` per user direction. Chronological is now a one-decision spec: which direction does the recency curve run? All count semantics dissolve into the continuous score.
- **Percentile rank over UNIQUE dates in the candidate pool, not the global corpus.** POOL_RERANKER semantics — sibling categories carve the pool; chronological ranks within. Tied dates → tied scores (same calendar day shares a percentile slot). Missing release_ts → score 0 (user instruction).
- **New module `db/chronological_scoring.py`** for the pure scoring math; mirrors the existing pattern of `db/metadata_scoring.py` / `db/vector_scoring.py` as a peer scoring helper. Returns dict[int, float] for the entire pool in one call (percentile needs to see all candidates at once, unlike per-movie vector/metadata scoring).
- **`ChronologicalQuerySpec` follows the ENTITY shape**: inherits directly from `EndpointParameters` with no nested `.parameters` field. Dispatch in `endpoint_executors.build_endpoint_coroutine` branches BEFORE the `params = wrapper.parameters` line, alongside ENTITY. Comment at that branch generalized to explain the pattern is now recognized.
- **Pool with <2 unique dates collapses to 1.0 for every valid-date movie.** No curve resolution to apply; equal credit beats an arbitrary 0 or 0.5. Missing-date movies still floor at 0.

### Planning Context
Earlier turns of this session staged two intermediate designs (top-N pick with `is_top_n_active` gate, then a final sort/slice over the rescored list). Both were rejected in favor of the qualifier-style continuous curve: top-N pick collides with the multi-channel rerank semantics; final slice introduces an arbitrary K-cutoff and special-cases what should just be another scoring contribution. Continuous percentile rank composes naturally with general_appeal / cultural_status / sibling carvers under the existing score-merge framework.

### Testing Notes
- `score_chronological` unit tests: tied dates, missing release_ts, single-unique-date pool, two-movie pool, large pool, both directions. Boundary values: oldest=1.0 for chronological, newest=1.0 for reverse_chronological; missing always 0.0.
- End-to-end via test_v3_endpoints.ipynb: confirm CHRONOLOGICAL fires under SINGLE_NON_METADATA_ENDPOINT bucket with the trimmed schema, the executor reaches Postgres and returns ScoredCandidate per pool member, no top-N artifacts appear.
- Query-shape verification: "newest Bond" (`reverse_chronological`), "earliest Kubrick" (`chronological`), "most recent Marvel" (`reverse_chronological`), "recent movies" (no-fire — routes to RELEASE_DATE).
- Watch: bucket-level prompt may still treat single-non-metadata routes as candidate-carvers in its objective wording; verify it doesn't push the LLM to coerce a direction for non-position phrases. The new `prompts/endpoints/chronological.md` should counterweight this, but the bucket objective `single_non_metadata_endpoint_objective.md` was not modified — re-read its wording during the first end-to-end test to confirm chronological-style abstention is supported.

## Cat 44 (CHRONOLOGICAL) — rename direction enum to OLDEST_FIRST / NEWEST_FIRST
Files: schemas/chronological_translation.py, db/chronological_scoring.py, search_v2/endpoint_fetching/category_handlers/prompts/endpoints/chronological.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/chronological.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/chronological.md, search_improvement_planning/query_categories.md
Why: Live testing showed the prior `chronological` / `reverse_chronological` enum values were doing all the disambiguation work via the prompt mapping ("oldest" → chronological) without the value names themselves carrying any signal. "Chronological order" has two competing English conventions (chronicle = oldest-first vs. modern-feed = newest-first), so a JSON output of `direction: chronological` reads ambiguously when inspected cold and risks the LLM flipping the mapping on phrasings the few-shot didn't cover.
Approach: Renamed the StrEnum values to OLDEST_FIRST = "oldest_first" and NEWEST_FIRST = "newest_first". The names now describe WHICH EXTREME OF THE POOL scores 1.0, eliminating the need to reason about sort direction. Updated the scoring helper's direction check, all three prompt files (endpoint contract, additional objective notes, few-shot), and the planning doc. Few-shot now has 5 fire examples covering: newest, oldest Kubrick, most recent Marvel, "sort by how long ago" + "oldest" (anti-pattern probe — verifies the LLM reads the surface phrase rather than the "sort" framing), earliest Star Wars. Plus the "recent" no-fire case retained.
Design context: Added an explicit mapping table to the endpoint prompt ("phrase contains X → direction = Y") and a strict reminder not to reason about "chronological order" / "sort direction". Single source of truth for the mapping is in three places intentionally — schema Field description, additional_objective_notes, and endpoint prompt — because the bucket prompt does not see all three at once depending on routing.
Testing notes: Re-run the diagnostic query from the user's live test ("oldest" with intent "Sort movies based on how long ago they were produced") and confirm direction is now `oldest_first`. Also verify: "earliest film" → oldest_first; "newest one" → newest_first; "most recent" → newest_first; "first Bond" (as in earliest) → oldest_first; "recent movies" → no-fire. Edge to watch: phrases that put "sort" / "chronological" wording in retrieval_intent without a clear extreme preference (e.g., "list these in chronological order") — endpoint prompt now instructs abstention for indeterminate cases.

## Cat 44 (CHRONOLOGICAL) — loosen firing threshold to fire-whenever-date-related
Files: search_v2/endpoint_fetching/category_handlers/prompts/endpoints/chronological.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/chronological.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/chronological.md
Why: Live test showed the LLM abstaining on a call it should have answered ("first" + intent "Sort movies based on how recent they were produced" → 0 endpoint specs generated). Prior prompt explicitly told the LLM to abstain when expression and intent didn't pin a clear extreme; combined with the bucket-level "declining to fire is preferred" guidance, the abstention threshold was too strict. User direction: fire whenever the call involves any date-direction preference; only abstain when the call has nothing to do with dates (effectively, only on upstream misroutes).
Approach: Rewrote three files. Endpoint prompt now leads with "fire whenever the call involves a date-direction preference of any kind — pick whichever direction best captures what the user wants" and added an explicit Conflict Resolution section (intent vs. expression disagreement → lean on the more-specific date-direction signal). Added "recent", "how recent", "modern", "current", "fresh" to the newest_first cue list (previously these would route to RELEASE_DATE without contributing here). Replaced the "abstain on indeterminate" rule with "abstain only when the call truly has no date signal — typically upstream misroute". Few-shot rewritten: dropped the strict no-fire case ("recent movies" was previously no-fire; now fires newest_first since "recent" IS a date-direction signal). Added the user's live debugging case as a worked conflict-resolution example: "first" + "how recent" → newest_first (intent dominates).
Design context: The previous abstention-first stance came from generic bucket-level guidance ("declining to fire is preferred"). For CHRONOLOGICAL specifically, that's the wrong default — the percentile curve degrades gracefully under direction mismatch (slightly mismatched still produces useful signal), but abstention loses the signal entirely. The recovered signal from broader firing outweighs the cost of occasionally picking the less-perfect direction. Boundary against Cat 13 (RELEASE_DATE) softened: the two categories can both fire on overlapping phrasings ("recent") since they're complementary (Cat 13 = range filter; Cat 44 = continuous direction curve).
Testing notes: Re-run the debugging query ("first" + "Sort movies based on how recent they were produced") and confirm direction=newest_first now emits (was 0 specs). Spot-check that abstention still works on truly date-unrelated calls if the upstream ever misroutes — those should still produce 0 specs. Watch for over-firing: if Cat 13 routes "recent" calls correctly upstream, CHRONOLOGICAL won't see them. If both fire on the same call, the rerank gets two contributions (range filter from Cat 13 + direction curve from Cat 44) which is the intended composition.

## NARRATIVE_SETTING — rewrite prompts around fragment-shape framework
Files: search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/narrative_setting.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/narrative_setting.md
Why: Prior guidance was vague ("phrase as setting description: 'set in...', 'takes place in...', 'during...'") and treated narrative setting as a flat space, leaving many real cases unhandled: fictional/abstract worlds, milieu/institutions, post-event situations, flashbacks, travel/relocation, and traversal (road-trip-style). Plot_summary embeddings key on the *grammatical surface form* of opening setting clauses in IMDB-style synopses, so the prompt should target those forms directly rather than describing setting as a concept.
Approach: Reorganized around two orthogonal dimensions. (1) Three setting-clause SHAPES based on how synopses actually open: Shape 1 coordinate anchor ("In [coords], ...") for time/place/era/milieu/environment; Shape 2 event-relative ("After/During [event], ...") for post-apocalyptic, war-during, stranded/trapped premises; Shape 3 world-establishing ("In [world] where [property], ...") for fictional/abstract worlds. Decision rule applied in order — first match wins — handles combos like "post-apocalyptic Tokyo" cleanly (Shape 2 first, place nested). (2) A POSITION dimension on top of the shape: opens-in (default), reaches (travel verbs), flashes-back-to (flashback signals), traverses (across/from-to). Output strategy: static input → emit position-neutral + opens-in concatenation; positional signal → emit only the matching position form to avoid diluting the embedding. Few-shot rewritten with 13 examples covering all three shapes × position variants + two no-fire boundaries (release date, filming location).
Design context: The fragment-shape framing came from rethinking what IMDB synopses actually look like as text rather than what setting "is" conceptually — different position framings produce distinctly different grammar in real synopses, and merging them into one fragment dilutes the embedding match. Boundaries vs. RELEASE_DATE (release era) and FILMING_LOCATION (production geography) preserved explicitly in the no-fire section.
Testing notes: Verify the user-facing cases: "1940s Berlin" → neutral + opens-in fragment pair; "high school" → opens-in with role-noun phrase; "post-apocalyptic" → event-relative with consequence; "magical realm" → world-establishing with rule property; "flashbacks to 1970s NYC" → flashes-back-to only; "travel to Paris" → reaches only; "road trip across the Midwest" → traversal + opens-in starter; "post-apocalyptic Tokyo" → combo resolves to Shape 2. Boundaries: "released in the 1940s" → no-fire (RELEASE_DATE); "filmed in Berlin" → no-fire (FILMING_LOCATION).


## STORY_THEMATIC_ARCHETYPE — lock semantic target to plot_analysis only
Files: search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/story_thematic_archetype.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/story_thematic_archetype.md
Why: Live behavior showed the category leaking into viewer_experience (and occasionally narrative_techniques) for thematic asks like "redemption arc" / "about grief". The prior prompt said "Fallback: Semantic plot_analysis" once but never explicitly ruled out other spaces, and the few-shot showed only routing decisions with no populated body — so the LLM had no anchor and translated thematic words into tonal vernacular (`emotional_palette.terms = ["grief", "heavy", "melancholy"]`), retrieving films that FEEL the theme rather than films that ARE the theme.
Approach: Rewrote both prompts. additional_objective_notes now (1) declares plot_analysis the EXCLUSIVE semantic target with `primary_vector` locked to plot_analysis, (2) names the two specific failure modes (drift to viewer_experience for emotional connotations; drift to narrative_techniques for arcs-as-craft) with reasoning for why each is wrong, (3) enumerates which sub-fields fire for which trait types (elevator_pitch / plot_overview / genre_signatures / conflict_type / thematic_concepts / character_arcs), (4) makes cross-field repetition mandatory and explains why (ingest side repeats load-bearing terms across four fields to weight the central concept; query side must match to land in the same neighborhood). Few-shot rewritten with three populated-body examples (redemption arc, kind of about grief, man-vs-nature survival) plus the two existing no-fire cases (CENTRAL_TOPIC, CHARACTER_ARCHETYPE), capped with a counter-example showing the viewer_experience drift mode and why it retrieves the wrong films.
Design context: Mirrors the pattern viewer_experience.md uses for single-space lock-in. The two drift modes both share a structural cause — thematic words have emotional/craft adjacencies that look populatable in those other spaces — so the prompt explicitly walks the LLM through why each adjacency is wrong rather than just listing rules.
Testing notes: Re-run thematic-archetype queries that previously leaked: "redemption arc", "about grief", "man-vs-nature survival", "underdog story", "coming-of-age" (when no tag exists). Verify (a) primary_vector is plot_analysis, (b) viewer_experience and narrative_techniques bodies are not populated, (c) the load-bearing thematic term appears across elevator_pitch / plot_overview / thematic_concepts / character_arcs. Spot-check that gradient phrasings ("kind of about grief") still route to Semantic even if a tag exists. Verify boundary no-fires still hold: "about JFK" → CENTRAL_TOPIC, "anti-hero" → CHARACTER_ARCHETYPE.

## NARRATIVE_SETTING — switch body form to paraphrase fragments, drop fabricated plot
Files: search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/narrative_setting.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/narrative_setting.md
Why: Prior body form (e.g. "Much of the film unfolds in 1940s Berlin. In 1940s Berlin, the story takes shape against the wartime backdrop.") included full-sentence narration plus role-noun and action fabrications ("a group of students navigate their senior year", "a lone trapper struggles against the cold", "survivors scavenge a ruined landscape"). The fabricated material pulls retrieval toward films that match the invented role/action rather than the broader pool of films set in the named coordinate, and the tonal padding ("wartime backdrop", "frozen wilderness") adds embedding mass on adjectives unrelated to the actual setting. The schema's `plot_summary` field description already prescribes the correct form for motif/setting queries — "short synopsis-prose fragments ... joined by periods ... WITHOUT fabricating a plot around it" — but the prompt was emitting the wrong shape.
Approach: Rewrote both files. Each shape (Shape 1 coordinate / Shape 2 event-relative / Shape 3 world-establishing) is now defined as a FAMILY of synonymous fragment paraphrases instead of a "[anchor], [plot fragment]" template. Shape 1 fragments: "In [X]. Set in [X]. Takes place in [X]. Much of the film unfolds in [X]. Against the backdrop of [X]." Shape 2 split into post/during/stranded sub-families with their own paraphrase vocabularies. Shape 3 keeps the "where [property]" form ONLY when the user grounded the property — never invented. Position dimension restructured the same way: each of opens-in / reaches / flashes-back-to / traverses now has its own paraphrase family rather than a single example fragment. Output strategy simplified to 4-6 fragments per body, all paraphrasing the same anchor. Few-shot rewritten (13 examples + 1 counter-example) — every body shows pure paraphrases with no role-nouns, no invented characters, no tonal adjectives. Added a "magical realm where wizards battle ancient evil" example to show how user-grounded properties propagate verbatim. Counter-example at the end shows the prior fabricated-plot form and walks through why it fails (drift toward fabricated role-noun/action, generic narration mass).
Design context: Reinforces the schema-level "NEVER fabricate plot detail" rule for motif/setting queries. Mirrors the discipline applied in viewer_experience.md (synonym density on the same axis, true paraphrases via substitution test). The fragment-family approach gives the embedding multiple ingest-side-shaped paraphrases of the SAME anchor to match against, which is the same logic ingest-side text uses (multiple ways of stating the same setting in the synopsis prose).
Testing notes: Re-verify user-facing cases produce paraphrase-only bodies: "1940s Berlin" → opens-in family with five paraphrases; "high school" → opens-in family, NO "a group of students" role-noun; "post-apocalyptic" → post-event sub-family, NO "survivors scavenge"; "magical realm" alone → Shape 3 type-paraphrases WITHOUT a property clause; "magical realm where wizards battle ancient evil" → Shape 3 WITH the property restated verbatim in every fragment; "flashbacks to 1970s NYC" → flashes-back-to family only; "travel to Paris" → reaches family only; "road trip across the Midwest" → traverses family without invented "small town to small town" specifics; "post-apocalyptic Tokyo" → Shape 2 fragments with Tokyo nested. Boundaries preserved: "released in the 1940s" / "filmed in Berlin" still no-fire.

## NARRATIVE_SETTING — restrict fragment vocabulary to forms attested in real synopses
Files: search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/narrative_setting.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/narrative_setting.md
Why: User flagged that the prior paraphrase forms ("Much of the film unfolds in X.", "Takes place in X.", "Against the backdrop of X.") are not how summaries are actually written. Sampled 16 real IMDB synopses + 14 plot_summary entries (Forrest Gump, Schindler's List, Lost in Translation, Casablanca, LOTR Fellowship, Inception, Mad Max Fury Road, The Road, Lincoln, Roma, Lady Bird, Mean Girls, 1917, Apocalypse Now, Saving Private Ryan, Little Miss Sunshine, Sideways, Easy Rider, Thelma & Louise, Nomadland) across the shape buckets and confirmed: real synopsis prose uses "In [X]." / bare "[X]." / "Set in [X]." / "During [event]." / "Following [event]." / "It's a post-[X] world." / "At the height of [event]." — and does NOT use "takes place in", "much of the film unfolds in", or "against the backdrop of" as bare fragments (those are marketing/critic-blurb grammar, not synopsis grammar). User direction after report: "Option C by far. Never fabricate additional details." — pure setting-anchor fragments only, restricted to attested grammar.
Approach: Replaced the paraphrase vocabulary with the attested-only fragment set. Shape 1 forms reduced to: "In [coords].", "[coords]." (title-card), "Set in [coords].", "In [year] [place].", "At [milieu].". Shape 2 split into post/during/stranded sub-families, each with its attested fragment list (added "It's a post-[event] world." from The Road's plot_summary; added "At the height of [event]." from 1917's plot_summary). Shape 3 keeps "In [world]." / "Set in [world]." / "In a [type] [world]." with type-paraphrase restricted to user-grounded vocabulary. Property clauses on Shape 3 still only emit when the user grounded the property, and the property is restated verbatim. Density dropped to 3-5 fragments per body (was 4-6) since the attested vocabulary is narrower. Added explicit "no critic-blurb phrasings" rule listing the three banned forms. Few-shot rewritten — every body uses only attested fragments. Two counter-examples at the end: one for invented plot, one for the unattested critic-blurb phrasings the prompt just banned.
Design context: The attested-vs-unattested distinction matters for cosine match because the embedded vector lands wherever the body's grammar lives in the embedding space; if the body uses metadata-caption grammar while ingest-side documents use synopsis grammar, the two land in different neighborhoods even when they're "about" the same setting. The substitution test still applies per-fragment: would this fragment plausibly appear inside a real IMDB synopsis? If no, drop it.
Testing notes: Re-run setting queries and verify bodies use only the attested forms. Specifically check that "Takes place in X.", "Much of the film unfolds in X.", and "Against the backdrop of X." never appear. Confirm: "1940s Berlin" → opens-in fragments using "In", "Set in", and bare title-card only; "post-apocalyptic" → five Shape 2 post-event fragments including "It's a post-apocalyptic world." and "In the aftermath of"; "during the Vietnam War" → during sub-family including "At the height of"; "magical realm" → Shape 3 without invented properties; "travel to Paris" → reaches family with verb-of-movement fragments; "road trip across the Midwest" → traverses family without invented town names. Boundaries unchanged: "released in the 1940s" / "filmed in Berlin" still no-fire.

## CULTURAL_STATUS — fire both popularity and reception for compound stature terms (cult classic)
Files: search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/cultural_status.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/cultural_status.md
Why: User flagged that "cult classic" was only firing the popularity column (popularity=niche), missing the reception=well_received signal that the "classic" half of the compound implies. Root cause: the notes file explicitly listed "cult" in the "weak or usually no metadata" bucket, and there was no few-shot for "cult classic" to demonstrate the compound decomposition — so the model landed cleanly on popularity (which owns "niche/underrated/lesser-known" per metadata.md) and abstained on reception.
Approach: Restructured the Metadata Augmentation section to distinguish three shapes: (1) single-concept stature ("classic", "iconic", "landmark", "culturally significant") where reception and popularity are substitutable evidence of one canonical-stature concept → scoring_method=ANY; (2) compound stature ("cult classic", "cult favorite", "beloved hidden gem") where the two columns are reinforcing facets of a compound term — both required — → scoring_method=ALL; (3) gap/polarization shapes ("underrated", "overhyped", "divisive", "ahead of its time") where the intent is a relationship between axes and scalars cannot express it → metadata abstains. Added a "cult classics" few-shot example demonstrating the compound decomposition (popularity=niche AND reception=well_received with scoring_method=ALL) and explicitly stating why a niche flop or a mainstream well-received film is NOT a cult classic. Also updated the existing "classic movies" few-shot's metadata_parameters line to make ANY-vs-ALL explicit (it was previously ambiguous "reception and/or popularity prior"). The "underrated thrillers" example remains unchanged — gap framing still abstains on metadata.
Design context: The compound-vs-substitutable distinction maps directly onto metadata.md's ANY-vs-ALL rule. A cult classic is fundamentally a two-axis description: low popularity + positive reception. Either axis alone is insufficient — a movie nobody watches that critics also hated is not a cult classic; a critically-praised mainstream hit is not a cult classic. So both columns fire and the score reflects how many of the two axes the candidate matches.
Testing notes: Re-run "cult classic" queries and verify the metadata call now fires both popularity=niche AND reception=well_received with scoring_method=ALL. Verify "classic movies" still fires both columns but with scoring_method=ANY. Verify "underrated thrillers" still abstains on metadata. Verify boundary categories unchanged: "cult classic" in financial_scale and specific_praise_criticism still no-fire (routes to cultural_status).

## Add /query_search (SSE) and /similarity_search (JSON) endpoints
Files: api/main.py, search_v2/streaming_orchestrator.py, search_v2/full_pipeline_orchestrator.py

### Intent
Expose the search pipeline over HTTP. /query_search streams progress as Server-Sent Events so the UI can render the fetch list, per-branch traits, and per-branch ranked results as each milestone completes (rather than waiting for the full pipeline). /similarity_search exposes the existing similarity flow keyed by caller-supplied TMDB IDs — used for "more like this" surfaces where the anchor movies are already known.

### Key Decisions
- Split `_run_branch` in full_pipeline_orchestrator.py into two reusable phases (`_run_step2_for_branch` → returns partial+QueryAnalysis, `_finish_branch_after_step2` → runs Step 3 + implicit policy). The existing `_run_branch` becomes a one-line composition so `run_full_pipeline` callers see no behavior change. This avoids adding a callback parameter, keeping cancellation semantics simple — every awaitable is constructed by the function that awaits it.
- New module `search_v2/streaming_orchestrator.py` owns the SSE-friendly orchestration: a transport-agnostic async generator yielding `(event_name, payload)` tuples. The endpoint in api/main.py is a thin wrapper that frames each tuple as SSE wire format.
- The streaming orchestrator runs a merge loop over `asyncio.wait(FIRST_COMPLETED)`, tagging each task by phase (step2/step3/stage4/nonstandard). Non-standard flows (exact-title, similarity) launch in parallel with standard-flow Step 2 tasks and emit a single `branch_results` event when done. Standard branches emit `branch_traits` after Step 2, then `branch_results` after Stage 4 + implicit-prior rerank.
- Stage 4 dispatch is gated on every standard branch having a final Step 2 outcome (success → Step 3 done, or failure → empty-traits placeholder), because `_build_auxiliary_specs` and `_apply_reranker_only_candidate_fallback` operate globally across the branch list. Within that gate, branches are still parallelized.
- New behavior vs run_full_pipeline: `/query_search` actually fires `run_exact_title_search` when Step 0 routes to it. The non-streaming orchestrator only surfaces the flag — for the streaming endpoint the user wants the exact-title fetch to produce real results.
- `branch_traits` payload deliberately carries only surface_text + polarity + commitment per trait. Decision was to emit immediately after Step 2 (before Step 3), so we don't include Step 3 category decomposition in the payload.
- `/similarity_search` calls `run_similar_movies_for_ids` directly (which already accepts tmdb_ids), avoiding a wasteful title→id roundtrip via `run_similarity_search`. ValueError → 400, LookupError → 422 (for unknown TMDB IDs).
- Serialization helpers are hand-written rather than `dataclasses.asdict()` to keep SSE payloads small (skips score_breakdowns and SimilarMoviesDebug, which are heavy diagnostic state).
- `finally:` block in the generator cancels every still-pending task — covers client disconnect (Starlette throws CancelledError into the generator) and any unexpected error path.

### Testing Notes
- Smoke test `/health` still passes (existing endpoint untouched).
- `/similarity_search` happy path: `{"tmdb_ids":[603]}` → 200 with `anchor_movie_ids:[603]` and a `ranked` list. Unknown ID → 422.
- `/query_search` smoke: `curl -N -X POST .../query_search -d '{"query":"sci-fi movies like Interstellar but cozier"}'` should stream `fetches_ready` → `branch_traits` per standard branch → `branch_results` per fetch → `done`.
- Exact-title routing: query like `"Inception"` should fire an exact_title fetch and emit its `branch_results`.
- Similarity routing inside `/query_search`: query like `"movies like Heat"` should emit a similarity fetch's `branch_results`.
- Edge cases to verify: empty query → 400; Step 0 failure → error event then done; per-branch Step 2 failure → branch_results with branch_error (no branch_traits for that fetch); client mid-stream cancel → server cleans up pending tasks (check logs for orphan-task warnings).

## CENTRAL_TOPIC: route semantic body to plot_analysis only
Files: search_v2/endpoint_fetching/category_handlers/prompts/categories/additional_objective_notes/central_topic.md, search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/central_topic.md
Why: CENTRAL_TOPIC handler was emitting meta-language plot_events bodies for topic-level queries (e.g. for "dog movie" it produced `"plot_summary": "dogs as central characters. dogs as primary subject matter. dogs as the focus of the story."`). That phrasing never appears in any stored plot_events text, so the embedding lands in a self-referential critic-review neighborhood rather than near actual dog-movie plots. The category target — concrete focal subject the movie orbits around — is topic/aboutness, not event-sequence; topic queries cannot land in plot_events without fabricating plot specifics, which biases retrieval to one trope.
Approach: Mirrored the story_thematic_archetype pattern. Restricted CENTRAL_TOPIC semantic routing to plot_analysis exclusively, with explicit reasoning for why each other space is wrong (plot_events would require fabrication; viewer_experience/narrative_techniques/etc. retrieve adjacency, not aboutness). Added sub-field selection guidance for the six plot_analysis sub-fields, the cross-field-repetition rule (load-bearing subject term must repeat across elevator_pitch / plot_overview / thematic_concepts), and density/register rules forbidding meta-descriptions of search intent. Rewrote all six few-shot examples with concrete JSON bodies and added a counter-example demonstrating the dog-movies plot_events failure mode.
Testing notes: Re-run handler on the queries used in current examples (WW2, biopics, running, Princess Diana, chess, death, dogs) and confirm semantic_parameters now contain only a plot_analysis entry with the subject repeated across fields. Watch for regressions where the LLM drifts into thematic abstractions (e.g. "loyalty" for dog movies) — those belong to STORY_THEMATIC_ARCHETYPE per the existing boundaries section.

## CENTRAL_TOPIC few-shots: viewer_experience trap, dogs, Antarctica
Files: search_v2/endpoint_fetching/category_handlers/prompts/categories/few_shot_examples/central_topic.md
Why: After the plot_analysis-only restructure, three gaps remained in the example coverage: (1) no example called out the viewer_experience drift trap that affects affect-laden subjects like "death" (LLM is tempted to emit `emotional_palette.terms = ["heavy", "melancholy"]` which retrieves films that FEEL heavy rather than films ABOUT death); (2) no place-as-subject example to mark the CENTRAL_TOPIC / NARRATIVE_SETTING boundary; (3) the dogs case (the canonical bug-spotting query) only lived as a counter-example, not a positive one.
Approach: Added a viewer_experience drift call-out to the death example using the same lesson story_thematic_archetype encodes for grief. Promoted dogs to a regular semantic-only example with a clean keyword-abstain rationale (`ANIMAL` too broad, `FAMILY` correlation-only) and a body emphasizing subject-position phrasing. Added an Antarctica example to cover place-as-subject and explicitly mark the boundary against NARRATIVE_SETTING ("the continent itself" framing distinguishes subject from backdrop). Reworked the counter-example header to acknowledge dogs now has a positive example and to generalize the failure mode to all CENTRAL_TOPIC traits, not just dogs.

## Hydrate /query_search and /similarity_search responses with MovieCard objects
Files: schemas/api_responses.py, db/postgres.py, api/main.py, search_v2/streaming_orchestrator.py

### Intent
Replace the score-keyed `[movie_id, score]` payloads in both endpoints' final results with arrays of frontend-ready cards: `{tmdb_id, title, release_date, poster_url}`. The UI needs renderable data, not just IDs.

### Key Decisions
- Added `MovieCard` Pydantic model in `schemas/api_responses.py` — minimal four-field shape. `release_date` is pre-formatted to ISO YYYY-MM-DD on the server (converted from `movie_card.release_ts`) so the client doesn't have to.
- Added `fetch_movie_card_summaries(movie_ids) -> list[MovieCard]` in `db/postgres.py` as a thin projection over the existing `fetch_movie_cards`. Preserves input order so the ranked sort stays intact. Silently drops tmdb_ids missing from `movie_card` (treat as not-yet-ingested).
- Introduced a uniform `_FetchOutcome` dataclass (cards + optional branch_error) so every per-fetch task wrapper (Stage 4 + implicit prior, similarity, exact-title) returns the same shape. The merge-loop handler now treats `_PHASE_STAGE4` and `_PHASE_NONSTANDARD` identically — one code path serializes both. Removes the per-result-type isinstance branching and the three separate serializer functions.
- Hydration is folded into each task wrapper rather than the merge-loop handler, so per-branch Postgres lookups parallelize instead of serializing through the single-consumer async generator. Same pattern as the earlier `_run_stage4_with_implicit_prior` refactor.
- SSE `branch_results` event payload is now `{fetch_id, results: list[MovieCard dict], branch_error}` — scores and evidence dropped per spec.
- `/similarity_search` returns a plain JSON array of MovieCard dicts (no anchor IDs, no scores, no evidence). Order is the descending-similarity order from `run_similar_movies_for_ids`.

### Testing Notes
- `/similarity_search` with a known tmdb_id (e.g., 603) should return an array of cards with non-null fields.
