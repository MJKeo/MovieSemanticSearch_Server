...
## Add per-category routing attributes to CategoryName enum
Files: schemas/trait_category.py, search_improvement_planning/v3_category_attributes.md
Why: Step 2's grounding LLM needs more than a one-paragraph description per category to route confidently — boundary statements, concrete edge cases, and worked examples sharpen the routing decision at category boundaries (about-vs-has, structural-vs-experiential, character-anchored-vs-not, etc.). All five fields are programmatically inserted into the step-2 system prompt, so attributes had to be on the enum members themselves rather than in an external doc.
Approach: Added four new fields to CategoryName (`boundary`, `edge_cases`, `good_examples`, `bad_examples`) alongside the existing `description`. Compressed each existing description down to a single core-definition sentence and lifted boundary content into its own field. Edge cases focus on the most common misroute traps; examples are distilled so each one illustrates a *distinct* routing pattern (no redundant illustrations). All 43 categories populated; verified imports cleanly and every category has every field non-empty. Companion planning doc v3_category_attributes.md captures the long-form rationale and flags open routing questions still needing user clarification.
Design context: Field design follows the v3_step_2_planning.md principle that reasoning fields sit ahead of decisions — boundary + edge_cases + bad_examples scaffold the routing decision before the LLM commits to a category label. Compactness is deliberate: prompt bloat hurts small-LLM performance.
Testing notes: No tests touched. Smoke-verified via uv run python that the enum constructs (43 members, all fields populated). The step-2 prompt builder will need updates to consume the new fields — tracked as a follow-up; nothing currently reads `description` outside of the (yet-to-be-rewritten) prompt assembly.

## Add CHARACTER_FRANCHISE category and fix MEDIA_TYPE endpoint regression
Files: schemas/trait_category.py
Why: Code review surfaced two issues from the prior CategoryName attribute pass: (1) MEDIA_TYPE silently lost its dedicated `EndpointRoute.MEDIA_TYPE` route — the rewrite collapsed it to `METADATA`, which would have bypassed the media_type dispatcher path entirely; (2) the planning doc's Cat 6 (character-anchored film franchises like Batman/Bond/Sherlock — one trait that fans out to ENT + FRA) didn't exist as an enum member, so step 2 had no clean home for those routings.
Approach: Restored MEDIA_TYPE's original `(EndpointRoute.MEDIA_TYPE,)` tuple. Inserted new CHARACTER_FRANCHISE member after FRANCHISE_LINEAGE with combo bucket and (ENTITY, FRANCHISE_STRUCTURE) endpoints — matches the planning-doc spec for the dual-nature referent. Updated NAMED_CHARACTER to non-anchoring-only ("Yoda, Hermione, Snape, Loki" — no longer covers Batman/Bond) and FRANCHISE_LINEAGE to character-less-only (MCU, Star Wars, LOTR — no longer covers Sherlock Holmes' franchise). Fixed every stale "NAMED_CHARACTER + FRANCHISE_LINEAGE" decomposition reference across the file: ADAPTATION_SOURCE Sherlock edge case, NAMED_SOURCE_CREATOR Sherlock bad_example and boundary, CHRONOLOGICAL "best Bond" bad_example, TITLE_TEXT boundary, ADAPTATION_SOURCE boundary. Also added two SEASONAL_HOLIDAY bad_examples ("documentary about Christmas" → CENTRAL_TOPIC, "snowed-in plot" → ELEMENT_PRESENCE/PLOT_EVENTS) per review finding.
Design context: Detection rule (per `search_improvement_planning/v3_category_attributes.md`) for routing between NAMED_CHARACTER / FRANCHISE_LINEAGE / CHARACTER_FRANCHISE: does a Wikipedia "List of [X] films" entity exist with the character as through-line? Yes → CHARACTER_FRANCHISE; non-anchoring character → NAMED_CHARACTER (with FRANCHISE_LINEAGE composing if franchise is named); character-less franchise → FRANCHISE_LINEAGE.
Testing notes: Smoke-verified 44 members load with all 7 fields populated. Endpoint+bucket pairs match the original spec for every category (one mismatch found and corrected during review: MEDIA_TYPE). Step 2 prompt builder still consumes only `.description` — the v3 prompt-builder rewrite that wires in `boundary` / `edge_cases` / `good_examples` / `bad_examples` is the next step and is being deferred deliberately.

## v3 step-2 trait taxonomy: review pass (1:1 trait→category, no internal jargon, multi-trait phrasing)
Files: schemas/trait_category.py

### Intent
Tighten the prompt-injected fields on `CategoryName` so step-2 produces clean 1:1 trait→category mappings and the LLM ingesting these definitions has zero project-internal jargon to decode.

### Key Decisions
- **Strict 1:1 trait→category invariant.** Removed every "single concept → multiple categories" pattern. Where a phrase legitimately decomposes into multiple traits, the example now spells it out as "X is two traits: 'A' → CAT_1 + 'B' → CAT_2" so the LLM never sees a single concept routed to two places. Affected: AWARDS, NUMERIC_RECEPTION_SCORE, GENERAL_APPEAL, SPECIFIC_PRAISE_CRITICISM, FINANCIAL_SCALE, RELEASE_DATE, RUNTIME, MEDIA_TYPE, SEASONAL_HOLIDAY, NARRATIVE_DEVICES, DIALOGUE_CRAFT_ACCLAIM, CHRONOLOGICAL, NAMED_SOURCE_CREATOR, LIKE_MEDIA_REFERENCE, MATURITY_RATING, TARGET_AUDIENCE, SENSITIVE_CONTENT.
- **Removed all internal acronyms and process language from prompt fields.** ENT/FRA/META/KW/SEM-style shorthand, "handler stage", "dispatcher default", "column", "salience=supporting downstream", "Step 2", "vector space", "prompt template", "modifier handling lives upstream" — none of these survive in description/boundary/edge_cases/good_examples/bad_examples. Section comments inside the class were also rewritten in plain prose. Constraint per user: definitions must be intelligible to someone with zero project context.
- **Lineage positioning is owned by FRANCHISE_LINEAGE for ALL franchise types.** Previously CHARACTER_FRANCHISE was described as fanning out to lineage internally; now its boundary explicitly says sequel/prequel/spinoff/reboot is a SEPARATE trait that splits out to FRANCHISE_LINEAGE. Concrete example: "Batman spinoffs" → 'Batman' (CHARACTER_FRANCHISE) + 'spinoffs' (FRANCHISE_LINEAGE). FRANCHISE_LINEAGE description and boundary were updated to match.
- **STUDIO_BRAND vs FRANCHISE_LINEAGE for ambiguous names is now context-driven, not defaulted.** Removed the "default to FRANCHISE_LINEAGE" rule; the LLM now picks the reading that best fits the surrounding query.
- **Stale FEMALE_LEAD reference fixed** in CHARACTER_ARCHETYPE — that category was never created; the trait stays in CHARACTER_ARCHETYPE itself with 'lone' splitting to NARRATIVE_DEVICES.
- **AWARDS / NUMERIC_RECEPTION_SCORE Oscar-worthy disambiguation** spelled out: aspirational praise → GENERAL_APPEAL; actual ceremony outcome verbs (won/nominated) → AWARDS.
- **Example diversification.** CHARACTER_FRANCHISE good_examples now (Batman, James Bond, Barbie) — three distinct facets (comics, literary multi-actor, toy/non-media). LIKE_MEDIA_REFERENCE good_examples expanded to 5 covering movie/TV/director/book/video-game referents to clearly differentiate stylistic comparison from "based on" adaptation language. TITLE_TEXT bad_examples gained a CHARACTER_FRANCHISE redirect. NAMED_CHARACTER good_examples gained "Loki appearances".
- **Removed "summer-blockbuster" from SEASONAL_HOLIDAY** — it's a release-window marketing term, not a holiday. Replaced the edge_case with a Christmas+GENRE composition.

### Testing Notes
Smoke-verified: 44 members load, all 7 fields populated on every category, no raw acronyms in any prompt-injected field, no unmarked single-trait→multi-category mappings remaining. Step 2 prompt builder still consumes only `.description` — the wiring of the four new fields into the prompt is still the deferred next step.

## v3 step-2 system prompt rewrite (fundamentals + programmatic taxonomy)
Files: search_v2/step_2.py, schemas/step_2.py

### Intent
Replace the legacy v2 prompt in `search_v2/step_2.py` (built around fragments + coverage_evidence + atomization patterns) with a v3-shape prompt aligned to the deployed `Step2Response` schema (`span_analysis` + `traits` with role/polarity/salience/category_candidates/best_fit_category). The legacy prompt no longer matched what the LLM was being asked to produce, so the rewrite is a hard cutover with no backwards compatibility.

### Key Decisions
- **Section ordering: task framing → atomicity → modifier vs trait → carver vs qualifier → polarity → salience → category taxonomy.** Fundamentals build bottom-up (you need a trait before you can classify it; you need role before you can assign salience). Categories sit closest to where they're used (the `category_candidates` / `best_fit_category` commit). Rationale documented in `search_improvement_planning/v3_step_2_planning.md` "Prompt context" section.
- **Reasoning-field discipline / out-of-scope reminders / worked end-to-end examples deliberately omitted.** Field-level reasoning guidance lives in the schema's per-field descriptions (each is a self-contained micro-prompt with PURPOSE / HOW TO THINK / FORBIDDEN / EXAMPLES). Repeating it in the prompt would create two sources of truth.
- **Sections written as guiding principles, not rule taxonomies.** Per project learning on small LLMs: principles + calibration examples generalize; flowcharts fail silently on inputs that don't match any branch. The polarity section explicitly tells the LLM to "read like a person" rather than memorize a closed list of polarity setters or distribution rules.
- **Programmatic category taxonomy rendering.** `_build_category_taxonomy_section()` rewritten to render five attributes per `CategoryName` enum value (description, boundary, edge_cases, good_examples, bad_examples). Per-category block format is fixed and parseable: `Cat: <name>` / `  Definition:` / `  Boundary:` / `  Edge cases:` / `  Good examples:` / `  Bad examples:`. The legacy "CLEAN fit / PARTIAL fit / NO_FIT" preamble is gone — fit_quality is no longer in the v3 schema; `fits` / `doesnt_fit` prose handles boundary comparison instead.
- **Salience reconciliation: hedges win.** The schema's prior `salience` field description claimed "a hedge does NOT automatically demote a headline want" with `'creepy' in 'ideally creepy and atmospheric horror'` mapping to CENTRAL. This contradicted explicit user direction in the v3 planning conversation (hedges are "an incredibly strong signal that should never be ignored" and downrank to supporting). Updated the schema field description to match: hedges are the primary signal in priority order, headline position is a clue not a rule and does not override hedges. The `'ideally creepy ...'` example flipped to SUPPORTING.
- **Executor and CLI sections untouched.** Same Gemini 3 Flash provider, no thinking, temp 0.35, same `run_step_2()` signature, same return shape. Module docstring updated to reflect the v3 schema shape.
- **Legacy constants deleted, not deprecated.** `_TASK_AND_OUTCOME`, `_LANGUAGE_TYPES`, `_CLARIFYING_EVIDENCE`, `_COVERAGE_EVIDENCE_RULES`, `_CHUNKING_RULES`, `_ATOMIZATION_PATTERNS`, `_GROUND_RULES`, `_OUTPUT_GUIDANCE` all removed. New constants: `_TASK_FRAMING`, `_ATOMICITY`, `_MODIFIER_VS_TRAIT`, `_CARVER_VS_QUALIFIER`, `_POLARITY`, `_SALIENCE`.

### Planning Context
Plan file: `~/.claude/plans/fizzy-watching-tome.md`. Source of truth for the fundamentals sections is `search_improvement_planning/v3_step_2_planning.md` "Prompt context: trait identification fundamentals" — sections were lifted near-verbatim with light prompt-shape adaptations. Section ordering rationale is in the same doc.

### Testing Notes
Hasn't been run yet — code changes only. Verification queries from the plan (cover the slip cases for each fundamental):
- `python -m search_v2.step_2 "popular movies"` — single-trait carver via the guiding principle (no other traits to qualify, so it must carve).
- `python -m search_v2.step_2 "ideally creepy and atmospheric 90s horror starring Anthony Hopkins"` — checks hedge-as-primary on salience (creepy + atmospheric should land SUPPORTING despite prominence).
- `python -m search_v2.step_2 "a non-violent crime thriller, not too dark or sad"` — carver-by-absence ("non-violent") vs negative qualifier ("dark", "sad" via "or"-distribution).
- `python -m search_v2.step_2 "iconic twist endings"` — must NOT split (scoped adjective creates compound).
- `python -m search_v2.step_2 "movies starring Hanks and Streep"` — multi-name compound DOES split per name.
- `python -m search_v2.step_2 "Stephen King novels"` — does NOT split mid-name; "novels" splits as separate trait.
