...
- **Observability:** observability_context/ — the as-built tracing system and its plan: `observability_architecture.md` (source of truth for what's instrumented today — spans, attributes, bootstrap, backends), `initial_implementation_context.md` (locked tooling decisions), `observability_todos.md` (phased build checklist + status), `observability_logs_plan.md` (goals and the *why*), `query_search_planning.md` (the /query_search instrumentation plan — per-phase spans/attributes, locked decisions, and a bite-sized implementation checklist). Read these whenever the user raises observability, tracing, OpenTelemetry/OTel, spans or span attributes, metrics, or logging — start with `observability_architecture.md`.
